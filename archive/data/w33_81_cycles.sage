...
e1 = V([1,0,0,0])  # First standard basis
e2 = V([0,1,0,0])  # Second
e3 = V([0,0,1,0])  # Third
e4 = V([0,0,0,1])  # Fourth
...
v = isotropic_vecs[0]
T = transvection_matrix(v, 1)
print(f"Transvection vector: {tuple(v)}")
print(f"Matrix T_{v,1}:")
print(T)

# Check order
print(f"T^3 = I: {T^3 == matrix.identity(F, 4)}")

# Find orbit of a point under T
test_point = isotropic_vecs[5]
orbit = [test_point]
current = test_point
for _ in range(10):
    current = normalize(current * T)
    if tuple(current) == tuple(orbit[0]):
        break
...
cycles_found = []
for v in isotropic_vecs[:20]:  # Sample first 20
    T = transvection_matrix(v, 1)
    if T^3 != matrix.identity(F, 4):
...
apartments_found = 0
for L1 in p0_lines[:5]:  # Sample some lines through p0
