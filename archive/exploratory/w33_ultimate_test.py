...
class W33UltimatePredictor:
    ...
    def derive_all_gauge_couplings(self):
        ...
        alpha_s_1 = self.k4 / (
            self.p * self.tri / self.p
        )  # Simplified: k4/tri × something
        ...
        unification_error = np.std([alpha_em_gut, alpha_s_gut, alpha_w_gut]) / np.mean(
            [alpha_em_gut, alpha_s_gut, alpha_w_gut]
        )
    ...
    def derive_all_fermion_masses(self):
        ...
        print(
            f"{'Particle':<8} {'Gen':<4} {'Type':<10} {'S':<8} {'Predicted':<15} {'Observed':<15} {'Ratio':<10}"
        )
        print("-" * 90)
        ...
        for name, info in sorted(fermions.items(), key=lambda x: -x[1]["obs"]):
            ...
            print(
                f"{name:<8} {info['gen']:<4} {info['type']:<10} {S:<8.3f} {m_pred:<15.4e} {m_obs:<15.4e} {ratio:<10.2f}"
            )
    ...
    def derive_higgs_parameters(self):
        ...
        return {"m_H": m_h_1, "v": best_v, "lambda": lambda_pred}
    ...
    def derive_pmns_matrix(self):
        ...
        theta_12_pred = np.degrees(
            np.arctan(np.sqrt(self.s3_classes[1] / self.s3_classes[0]))
        )
        ...
        print(
            f"  vs observed {theta_12_obs:.2f}° (error: {abs(tbm_12 - theta_12_obs)/theta_12_obs*100:.2f}%)"
        )
    ...
    def derive_cosmological_parameters(self):
        ...
        error_lambda = (
            abs(omega_lambda_pred - omega_lambda_obs) / omega_lambda_obs * 100
        )
    ...
    def final_scorecard(self):
        ...
        print()
        print(
            f"Results: {excellent} excellent (<10%), {good} good (<25%), {moderate} moderate (<50%), {poor} poor (>50%)"
        )
