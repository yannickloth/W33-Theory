...
for p in particles[:10]:  # First 10
    print(f"{p.name:<20} {p.symbol:<8} {p.mass_gev:<12.4f} {p.charge:<8.2f} {p.spin:<6.1f}")
print("...")
...
print("\nInteraction Summary:")
print(f"  Electromagnetic: {sum(1 for i in interactions if i.interaction_type == 'EM')}")
...
def decay_rate(parent: Particle, daughters: List[Particle], w33: W33) -> float:
    ...
    mass_factor = max(0, parent.mass_gev - daughters[0].mass_gev - daughters[1].mass_gev)
        ...
        if Z:
            ...
            for p in particles:
                if p.is_fermion and 2 * p.mass_gev < available_energy:
                    ...
                    k = (w33.get_phase(v1, v2) + w33.get_phase(v2, v3) + 
                         w33.get_phase(v3, v1)) % 12
