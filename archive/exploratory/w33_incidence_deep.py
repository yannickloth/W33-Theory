...
off_diag_vals = [
    off_diag[i, j] for i in range(n_points) for j in range(i + 1, n_points)
]
...
off_diag_l_vals = [
    off_diag_l[i, j] for i in range(n_lines) for j in range(i + 1, n_lines)
]
...
print(
    f"Expected for GQ(3,3): Each point has 4×3=12 collinear points, so 40-1-12=27 non-collinear"
)
