...
class W33BeyondStandardModel:
    ...
    def predict_susy_scale(self):
        ...
        print(
            f"  M_SUSY = √{self.pgu} × 10 = {m_susy_2:.2e} GeV = {m_susy_2/1e3:.1f} TeV"
        )
        ...
        print(
            f"  M_SUSY = {self.m_gut:.2e} / 22 = {m_susy_3:.2e} GeV = {m_susy_3/1e3:.1f} TeV"
        )
        ...
        print(
            f"  - Gluino pair production at √s > {2*sparticles['gluino']*m_susy_best/1e3:.0f} TeV"
        )
    ...
    def predict_black_hole_entropy(self):
        ...
        print(
            f"  - Total: log₂({self.tri}) = {np.log2(self.tri):.2f} bits per Planck area"
        )
        print(f"  - Holographic principle preserved!")
        print()
        print(
            "  - Tricentric triangles ({}) = observer-accessible info".format(self.tc)
        )
    ...
    def predict_quantum_gravity_scale(self):
        ...
        return {"lambda_qg_gev": lambda_qg_w33, "l_min_cm": l_min}
