...
class W33CKMPredictor:
    ...
    def predict_from_triangle_ratios(self):
        ...
        print(f"Tricentric fraction: f = {self.tricentric}/{self.triangles} = {f_tri:.6f}")
        print()
        ...
        print(f"  θ₁₂ = arctan(√{self.q45/self.k4:.6f}) = arctan({np.sqrt(self.q45/self.k4):.6f})")
    ...
    def predict_from_automorphism_orbits(self):
        ...
        error_12_points = abs(theta_12_points - self.theta_12_obs) / self.theta_12_obs * 100
        
        print(f"From 40 points:")
        print(f"  θ₁₂ = arctan(1/√40) = arctan(1/{np.sqrt(40):.3f}) = {theta_12_points:.2f}°")
    ...
    def predict_jarlskog_invariant(self):
        ...
        J_pred = np.sin(t12) * np.cos(t12) * np.sin(t23) * np.cos(t23) * np.sin(t13)**2 * np.sin(d)
        
        error = abs(J_pred - J_obs) / J_obs * 100
        
        print(f"Prediction using W33 angles:")
        print(f"  θ₁₂ = {theta_12:.2f}°, θ₁₃ = {theta_13:.3f}°, θ₂₃ = {theta_23:.2f}°, δ = {delta}°")
