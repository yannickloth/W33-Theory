...
for i in range(n_irreps):
    chi_i = list(irreps[i])
    d = int(chi_i[0].IsInt() and int(chi_i[0]) or round(complex(str(chi_i[0].sage())).real))
    irrep_dims.append(d)
...
for i in range(n_irreps):
    ...
    if abs(mult_approx) > 0.1 or dim_i == 81:
        print(f"  Irrep {i:2}: dim={dim_i:3}, <chi_H1,chi_{i}>={mult_approx:8.4f}, mult={mult}")
...
if total_dim == h1_dim:
    print("\n✓ H1 decomposes as:")
    terms = [f"{mult}×V_{idx}({deg}-dim)" if mult > 1 else f"V_{idx}({deg}-dim)" 
             for idx, deg, mult in decomposition]
    print("  H1 = " + " ⊕ ".join(terms))
