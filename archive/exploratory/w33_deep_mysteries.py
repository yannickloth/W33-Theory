...
class W33DeepStructure:
    ...
    def investigate_11_connection(self):
        ...
        print(
            f"  TRIANGLES / (2×11) = {self.TRIANGLES // 22} = {self.TRIANGLES / 22:.1f}"
        )
    ...
    def analyze_17_exact_rationals(self):
        ...
        print(
            f"Most common denominator: {max(set(denominators), key=denominators.count)}"
        )
        print()
    ...
    def find_fundamental_constants(self):
        ...
        print(
            f"  α ≈ 1/(2π e^S_max) = 1/(2π × e^{S_max:.3f}) = {alpha_from_entropy:.6f}"
        )
        ...
        for name1, val1 in [("TRICENTRIC", self.TRICENTRIC), ("Q45", self.Q45)]:
            for name2, val2 in [
                ("TRIANGLES", self.TRIANGLES),
                ("K4*TRIANGLES", self.K4 * self.TRIANGLES),
            ]:
                ratio = (val1 / val2) ** 2
        ...
        return {"alpha_estimate": alpha_from_entropy, "ratio_137_22": 137 / 22}
    ...
    def unification_energy_scales(self):
                ...
                if scale1_name != scale2_name:
                    ...
                    for w33_name, w33_ratio in w33_ratios.items():
                        if 0.8 < ratio / w33_ratio < 1.2:
                            ...
                            print(
                                f"  {scale1_name}/{scale2_name} = {ratio:.3f} ≈ {w33_name} (error: {error:.1f}%)"
                            )
        ...
        scale_list = sorted(
            [(name, scale) for name, scale in scales.items()], key=lambda x: -x[1]
        )
