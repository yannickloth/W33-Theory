...
class W33DeepestMysteries:
    ...
    def hierarchy_problem(self):
        ...
        print("-" * 90)
        print(
            f"Why is M_Higgs = {self.m_higgs} GeV << M_Planck = {self.m_planck:.2e} GeV?"
        )
        ...
        print(
            f"  M_Higgs / M_Planck = {self.tc} / {self.tri} = 1/{self.tri/self.tc:.1f}"
        )
        ...
        print(
            f"  1. Observable space (240 tricentric) is a TINY fraction of total ({self.tri})"
        )
    ...
    def cosmological_constant_problem(self):
        ...
        print(
            f"  Suppression: exp(-(S_tot - S_obs)) = exp(-{S_total - S_observable:.3f})"
        )
    ...
    def arrow_of_time(self):
        ...
        print()
        print(
            f"When S = S_max, we reach HEAT DEATH (all {self.tri} states equally likely)"
        )
        print()
