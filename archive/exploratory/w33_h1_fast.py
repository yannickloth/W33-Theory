...
if dim_B1 > 0 and dim_Z1 > 0:
    ...
    full_basis = np.hstack([B1_orthonormal, np.random.randn(dim_Z1, dim_Z1 - rank_B1_in_Z1)])
    Q_full, _ = np.linalg.qr(full_basis)
    
    H1_basis_in_Z1 = Q_full[:, rank_B1_in_Z1:rank_B1_in_Z1 + beta1]  # Shape: (dim_Z1, beta1)
else:
...
if chi_squared >= 0.99:
    ...
    for i in range(len(irreps)):
        ...
        dim_i = int(irrep_chi_list[0].IsInt() and int(irrep_chi_list[0]) or 
                    round(complex(str(irrep_chi_list[0].sage())).real))
