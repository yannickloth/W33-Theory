...
for c_idx in range(n_classes):
    ...
    print(f"  Class {c_idx}: size={gap_class_sizes[c_idx]:5}, order={order_elem:2}, chi={h1_chi[-1]}")

print(f"\nH1 character vector: {h1_chi}")

# Now compute <chi, chi> using GAP class sizes
chi_squared = sum(gap_class_sizes[j] * h1_chi[j]**2 for j in range(n_classes)) / group_order
