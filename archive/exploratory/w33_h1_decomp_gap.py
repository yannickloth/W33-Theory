...
def main():
    ...
    from sage.all import QQ, Graph, gap, libgap, matrix
    ...
    h1_mats = [
        matrix(QQ, [[QQ(x) for x in row] for row in mat_str]) for mat_str in h1_mats_str
    ]
    dim = h1_mats[0].nrows()
    ...
    for i in range(n_classes):
        ...
        print(
            f"  Class {i}: size={class_sizes[i]:5}, order={int(rep_gap.Order()):3}, chi={chi_val}"
        )
    ...
    for i in range(n_irreps):
        ...
        irrep_deg = int(
            irrep_chi[0].IsInt()
            and int(irrep_chi[0])
            or int(irrep_chi[0].IsRat() and float(irrep_chi[0]) or 0)
        )
        if irrep_deg == 0:
