...
\subsection{$q=11$: provisional $\Omega_{11}$ pairing (executed, non-canonical)}

We constructed the $q=11$ local generator shell (96,624 elements) and found an order-3 semiregular projectivization element on the shell, yielding 32,208 disjoint 3-cycles. Using these orbits, we produced a deterministic list of 16,104 classes by sorting the 3-cycle orbits by a canonical representative and pairing consecutive orbits. This achieves the target cardinality $|\Omega_{11}|=16104$ and provides a concrete working dataset.

\begin{remarkbox}
This pairing is not yet the canonical $\mathbb{Z}_2$ quotient predicted by the odd-$q$ theory: the remaining step is to construct an explicit nonsquare-determinant PGL/PSL involution acting on the shell (the global sign gauge), which should induce a natural 2-to-1 pairing of the 3-cycle orbits. (Bundle: \texttt{W33\_q11\_Omega11\_provisional\_bundle.zip}.)
\end{remarkbox}
...
\subsection{$q=11$: canonical PGL nonsquare involution and dihedral search (executed)}

We constructed a canonical $\mathbb{Z}_2$ involution $f$ on the $q=11$ generator shell by propagating a fixed nonsquare $PGL(2,11)$ element (e.g. inversion $x\mapsto 1/x$ on $PG(1,11)$) to each point pencil via conjugation by the computed pencil transports. This yields a genuine involution on local line-pair generators and hence on the 96,624 shell elements in $H_{11}$.

We then searched for an order-3 shell projectivization $M_3$ satisfying the dihedral normalization condition
\[
f\circ M_3 \circ f = M_3^{-1},
\]
which would force the induced involution on the 32,208 $C_3$-orbits to decompose purely into 2-cycles (a fully canonical $\Omega_{11}$ pairing). In the current 14-generator search window we did not find such a dihedral-normalized $M_3$; the induced orbit graph under $f$ has components of varying sizes. A diagnostic bundle includes the orbit-component size distribution and a large sample of the involution map.

(Bundle: \texttt{W33\_q11\_dihedral\_search\_results\_bundle.zip}.)




% =====================================================================
\section{Executed q=11 closure: $\Omega_{11}$ as $D_6$-orbits and the rank-4 base scheme}
\label{sec:q11_executed_closure}
% =====================================================================

This section records the fully executed q=11 closure mechanisms.

\subsection{Canonical $\Omega_{11}$ as a dihedral quotient on the shell}
Let $\mathcal{S}_{11}$ be the $q=11$ shell (size $96{,}624$). The intrinsic order-3 action $X$ partitions $\mathcal{S}_{11}$ into $32{,}208$ 3-cycles.
Using the global sign gauge $\varepsilon$ and the shell label $\chi$, we constructed an involution $Y$ on shell elements with a forced cycle-phase alignment.
The group $\langle X,Y\rangle$ acts dihedrally ($D_6$), and every orbit has size $6$.
Therefore
\[
\Omega_{11} \;=\; \mathcal{S}_{11}/\langle X,Y\rangle,\qquad |\Omega_{11}|=96{,}624/6 = 16{,}104.
\]
This replaces provisional pairings with a genuine group quotient.

\subsection{Canonical point$\times 11$ fiber assignment}
Each $\Omega_{11}$ element has point support of size $1$ or $3$.
We solved a bipartite b-matching (maxflow) assigning each $\Omega_{11}$ element to exactly one of its support points, with capacity $11$ per point.
This yields a canonical incidence
\[
\Omega_{11}\cong PG(3,11)\times\{0,\dots,10\},
\]
derived from the $D_6$ quotient (not assumed).
The fiber label is defined by sorting the 11 assigned elements per point by the canonical representative integer.

\subsection{Rank-4 base scheme on $\Omega_{11}$}
Ignoring the selector transport on the 11-fiber, there is an exact rank-4 association scheme on $\Omega_{11}$ defined solely by basepoint incidence:
\begin{itemize}
\item $R_0$: identity,
\item $R_1$: same point, different fiber label (valency 10),
\item $R_2$: distinct points collinear in $W(3,11)$ (valency 1452),
\item $R_3$: distinct points noncollinear (valency 14641).
\end{itemize}
We verified exact intersection constancy for this rank-4 partition.
The remaining q=11 work is to refine $R_3$ into a gauge-covariant selector relation using lifted transport / center projectors, rather than the naive scalar shift $b-a\equiv \omega(u,v)$.
