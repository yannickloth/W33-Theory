...
- If you want the torus/Fano side first, read `Möbius torus seed`, then `Surface congruence selector`, then `Decimal / Fano affine split`, then `Szilassi dual`, then `Heawood harmonic bridge`, then `Heawood / Klein symmetry`, then `Heawood shell ladder`, then `Surface / Hurwitz flag shell`, then `Surface / Physics Shell`, then `Klein Hurwitz Extremal Lift`, then `Hurwitz 2·3·7 Selector`, and then `Surface/Fano bridge`.
