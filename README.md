...
- Post-enumeration tooling now supports exhaustive-wrapper output, canonical-rep geotype
  classification, comparative census reporting, and image-gallery generation:
  `tools/enumerate_minimal_certificates_exhaustive.py`, `tools/classify_canonical_reps.py`,
  `tools/report_min_cert_census.py`, `tools/make_min_cert_gallery.py`.
- Exact census now exposes an orbit-stabilizer split in the Hessian layer:
  orbit sizes split into `1296` and `2592` (implied stabilizer sizes `2` and `1`),
  while full `AGL(2,3)` representatives are all in the full-size `2592` orbit class.
- Repro path:
  run `tools/build_e6_f3_trilinear_map.py`, then `tools/analyze_e6_f3_trilinear_symmetry_breaking.py`, then `tools/enumerate_minimal_certificates.py` (sampling or `--mode exact`), then `tools/enumerate_minimal_certificates_exhaustive.py`, then `tools/classify_canonical_reps.py`, then `tools/report_min_cert_census.py`, then `python -m pytest tests/test_e6_f3_trilinear.py tests/test_e6_f3_trilinear_symmetry_breaking.py tests/test_witness_certificate_classification.py tests/test_enumerate_minimal_certificates_smoke.py tests/test_enumerate_minimal_certificates_exhaustive_smoke.py tests/test_classify_canonical_reps_smoke.py tests/test_report_min_cert_census_smoke.py tests/test_make_min_cert_gallery_smoke.py -q`.
...
| `E6/F3 trilinear sign layer` | finite-field cubic sign laws + residual symmetry certificates + dual-space full-sign obstruction (`7` witnesses) + residual orbit fingerprint (`[1,2,6]`/`[1,2,3,6]`) + striation action (`[1,3]` with non-distinguished `S3`) + exact flag-line class decomposition (`1/2/3/6`) + distinct-line obstruction split (`7` vs `8`) + striation-complete obstruction split (`7` vs `8`) + minimal-certificate geometry/orbit diagnostics + exhaustive Hessian witness-orbit census (`256` canonical reps over `273` covering combinations) + canonical-rep census/gallery tooling + orbit-stabilizer stratification (`1296/2592` in Hessian, `2592` only in AGL) | `tools/build_e6_f3_trilinear_map.py`, `tools/analyze_e6_f3_trilinear_symmetry_breaking.py`, `tools/enumerate_minimal_certificates.py`, `tools/enumerate_minimal_certificates_exhaustive.py`, `tools/classify_canonical_reps.py`, `tools/report_min_cert_census.py`, `tools/make_min_cert_gallery.py` | `tests/test_e6_f3_trilinear.py`, `tests/test_e6_f3_trilinear_symmetry_breaking.py`, `tests/test_witness_certificate_classification.py`, `tests/test_enumerate_minimal_certificates_smoke.py`, `tests/test_enumerate_minimal_certificates_exhaustive_smoke.py`, `tests/test_classify_canonical_reps_smoke.py`, `tests/test_report_min_cert_census_smoke.py`, `tests/test_make_min_cert_gallery_smoke.py` | `artifacts/e6_f3_trilinear_map.json`, `artifacts/e6_f3_trilinear_symmetry_breaking.json`, `artifacts/e6_f3_trilinear_min_cert_enumeration_hessian_exhaustive2.json`, `docs/NOVEL_CONNECTIONS_2026_02_10.md`, `docs/README_EXTENSION_ONLINE_FINDINGS_2026_02_10.md` |
...
python tools/enumerate_minimal_certificates.py --in-json artifacts/e6_f3_trilinear_map.json --candidate-space agl --mode exact --out-json artifacts/e6_f3_trilinear_min_cert_exact_agl_full.json
python tools/enumerate_minimal_certificates_exhaustive.py --in-json artifacts/e6_f3_trilinear_map.json --candidate-space hessian --out-json artifacts/e6_f3_trilinear_min_cert_exhaustive_wrapper_hessian_full.json
python tools/classify_canonical_reps.py --in-json artifacts/e6_f3_trilinear_min_cert_exhaustive_wrapper_hessian_full.json --out-json artifacts/e6_f3_trilinear_min_cert_classified_hessian_full.json
python tools/classify_canonical_reps.py --in-json artifacts/e6_f3_trilinear_min_cert_exact_agl_full.json --out-json artifacts/e6_f3_trilinear_min_cert_classified_agl_full.json
python tools/report_min_cert_census.py --hessian-json artifacts/e6_f3_trilinear_min_cert_classified_hessian_full.json --agl-json artifacts/e6_f3_trilinear_min_cert_classified_agl_full.json --out-json artifacts/e6_f3_trilinear_min_cert_census_report.json --out-md docs/MIN_CERT_CENSUS_REPORT_2026_02_10.md
python tools/make_min_cert_gallery.py --out-md docs/MIN_CERT_REPRESENTATIVE_GALLERY_2026_02_10.md --out-dir reports/min_cert_census/gallery --per-dataset 9
