...
The sharpest promoted closure on `master` is now the **continuum tail syzygy
criterion** statement of `CCCXC`: the promoted observable shell
...
data is already generated by one scalar amplitude `A=217` on the fixed
primitive coefficient ray `(65,662)`, any one promoted nonzero witness already
recovers that same scalar, and that witness criterion now forces one unique
normalized tail operator profile `(65,662,15650/3,17993/4)` on the fixed
avatar. That fixed line is now cut out directly by three exact syzygies on the
avatar coordinates themselves, so the remaining wall is exact projective
tail-line membership plus forced nonzero normalization from genuine external
realization data.
