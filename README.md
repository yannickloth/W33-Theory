...
- **Yukawa reduction:** the Yukawa side is now exact at scaffold level and reduced one step further: clean Higgs pair `H_2, Hbar_2`, slot-independent V4 label matrix `[[AB,I,A],[AB,I,A],[A,B,0]]`, rigid `2+2` / `1+3` right-support split, projected CE2 rank `28` versus full arbitrary-screen rank `36` with nullity `0`, and after the V4 split each active sector becomes an exact two-template block span driven by a universal commuting unipotent `3x3` generation matrix. The remaining Yukawa frontier is the slot-specific nonzero internal template spectrum
