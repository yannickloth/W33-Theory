...
The external K3 bridge is now sharper than that long summary alone suggests. Globally the canonical minimal carrier is `U1`, but locally the selector packet is dominated by `U3` on the hyperbolic side and by `E8_2` on the exceptional side. The internal family flag is correspondingly sharper than the current external realization: internally the reduced Yukawa packet has exact line-plane data `span(1,1,0) < {x = y}` with common square `2E13`, while externally the current bridge fixes only the carrier plane `U1` and the graded shadow `81 ⊕ 81`. So the present exact K3 bridge sees the carrier plane and graded shadow of the central `2E13` family channel, but not yet a canonical external line or a non-split external extension object.
