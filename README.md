...
- **Fermion hierarchy Rosetta:** the tree electromagnetic count `137` and the first up-sector suppressor `136 = lambda mu (mu^2+1)` are now one exact graph packet, so `m_c/m_t = 1/(137-1) = 1/136`, while the down-sector ladder closes as `m_b/m_c = 13/4`, `m_s/m_b = 1/44`, `m_d/m_s = 1/20`, and the charged-lepton shell carries `m_mu/m_e = 208`
