#!/usr/bin/env python3
"""
V30 — Bucketed/out-of-core exact l9 = -δl8 (compact records; stats-first).

This is the same computation as V27, but with a more disk/memory-friendly
record format so that a full exact l9 run is feasible on a 16GB machine.

Pipeline:
  (A) MAP:    stream l8 JSONL and emit per-(t9, out) contribution records into hashed buckets
  (B) REDUCE: for each bucket, aggregate contributions per t9 and report final l9 stats

Differential (same convention as V24/V25/V26/V27):
...
Key implementation details:
  ...
  - MAP expands each l8 entry only along g1 neighbors of its g2 output (exactly 19),
    not along all 73 possible insertions.

Compact bucket record format (fixed-size, little-endian):
    packed_t9 (9 bytes; 9×uint8, basis indices 0..247) +
    out_idx   (uint8) +
    coeff     (int8)   # per-record contribution, guaranteed in [-126,126] for this run
Total: 11 bytes/record.

By default we run "stats-only" (no JSONL output table), since full l9 is expected
to be billions of nonzero entries. You can enable JSONL output for small pilots.
"""
...
DEFAULT_OUT_DIR = ROOT / "V30_output_v13_full"

Pair = Tuple[int, int]
Terms = Tuple[Tuple[int, int], ...]  # tuple[(basis_index, coeff)]

# Compact binary bucket record encoding
PACK8 = struct.Struct("<8B")
REC_TAIL = struct.Struct("<Bb")  # out_idx:uint8, coeff:int8
KEY_LEN = 9
REC_SIZE = KEY_LEN + REC_TAIL.size  # 9 + 2 = 11 bytes


def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="V30: compact bucketed exact l9 = -δl8 (out-of-core).")
    ...
    ap.add_argument(
        ...
        type=int,
        default=4096,
        help="Number of hash buckets (recommended: power of 2). Full-run default: 4096.",
    )
    ap.add_argument(
        "--max-lines",
        type=int,
    ...
    ap.add_argument(
        "--write-jsonl",
        action="store_true",
        help="Write l9 JSONL output (ONLY recommended for small pilots).",
    )
    ap.add_argument(
        "--progress-every",
        type=int,
...
def map_stage(
    *,
                ...
                for out_idx, coeff in l8_terms:
                    ...
                    if nbrs is None:
                        raise RuntimeError(
                            ...
                            "this MAP assumes l8 outputs are g2 roots."
                        )
                    ...
                    for g, br_terms in nbrs:
                        ...
                        packed_t9 = packed_t8[:pos] + bytes([g]) + packed_t8[pos:]
                        ...
                        for out2, bcoeff in br_terms:
                            ...
                            if v < -128 or v > 127:
                                raise OverflowError(
                                    f"Per-record coeff overflow at line {line_no}: v={v} "
                                    f"(coeff={coeff}, bcoeff={bcoeff})"
                                )
    ...
    return {
        ...
        "record_size_bytes": int(REC_SIZE),
    }
    ...
    try:
        for b in range(bucket_start, bucket_end):
            ...
            with p.open("rb") as f:
                while True:
                    ...
                    for off in range(0, len(buf), REC_SIZE):
                        key = bytes(mv[off : off + KEY_LEN])
                        out_idx, coeff = REC_TAIL.unpack_from(mv, off + KEY_LEN)
                        n_records += 1
                        ...
                        if cur is None:
                            if out_idx < 8:
                                ...
                                acc[key] = [out_idx, int(coeff)]  # mutable for faster +=
            ...
            for key, val in acc.items():
                n_unique += 1
                t9 = list(key) if fout is not None else None
...
def main() -> None:
    ...
    report_json = out_dir / "v30_bucketed_deltaL8_to_l9_report.json"
    ...
    print("=" * 78)
    print("V30 — Compact bucketed exact l9 = -δl8")
    print("=" * 78)
    ...
    print(f"write jsonl:    {bool(args.write_jsonl)}")
    ...
    if args.stage in ("reduce", "all"):
        print("\n[2/2] REDUCE stage: aggregating buckets → l9 stats ...")
