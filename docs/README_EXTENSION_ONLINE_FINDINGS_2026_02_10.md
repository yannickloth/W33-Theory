...
- Randomized enumeration (greedy sampler) results: the sampler was extended with parallel sampling, early-stopping, and checkpointing support (`--workers`, `--batch-size`, `--patience`, `--checkpoint-interval`, `--stop-if-found`). Running a medium sweep (20k samples, `--workers 4 --batch-size 1000 --checkpoint-interval 2000`) produced: Hessian216: **134** distinct canonical representatives (`artifacts/e6_f3_trilinear_min_cert_enumeration_hessian_20k.json`); AGL(2,3): **7** distinct canonical representatives (`artifacts/e6_f3_trilinear_min_cert_enumeration_agl_20k.json`). These results indicate a much larger variety of minimal witness orbit types in the Hessian candidate space compared to AGL space.
