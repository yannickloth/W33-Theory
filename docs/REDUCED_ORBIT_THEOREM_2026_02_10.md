...
- Core-motif orbit polarization: `tools/classify_core_motif_orbit_polarization.py` (writes `artifacts/core_motif_orbit_polarization_2026_02_11.json` and `docs/CORE_MOTIF_ORBIT_POLARIZATION_2026_02_11.md`).
- Core-motif enrichment stats: `tools/core_motif_enrichment_stats.py` (writes `artifacts/core_motif_enrichment_stats_2026_02_11.json` and `docs/CORE_MOTIF_ENRICHMENT_STATS_2026_02_11.md`).
- Core-motif anchor channels: `tools/core_motif_anchor_channels.py` (writes `artifacts/core_motif_anchor_channels_2026_02_11.json` and `docs/CORE_MOTIF_ANCHOR_CHANNELS_2026_02_11.md`).
- Core-motif anchor-set search: `tools/search_core_motif_anchor_sets.py` (writes `artifacts/core_motif_anchor_search_2026_02_11.json` and `docs/CORE_MOTIF_ANCHOR_SEARCH_2026_02_11.md`).
...
- The above proof is a concise, canonical algebraic argument augmented by exhaustive computational verification on the canonical Hessian representative census. The finite-case reduction to exclude `z=(2,2)` has now been implemented and machine-checked (`tools/prove_exclude_z22.py`, `tests/test_prove_exclude_z22_smoke.py`, `tests/test_formal_proof_z22.py`), and a global closed-form scan now confirms zero stabilizers for `z=(2,2)` in full `AGL(2,3)` as well (`tools/prove_z22_no_global_stabilizer.py`, `tests/test_prove_z22_no_global_stabilizer_smoke.py`). The symbolic module now also reports the full `z in {0,1,2}` vertical-line profile and confirms that no fixed point of `z -> 2*z+2` can satisfy `P(L)=s(L,z)` (`tools/formal_z22_proof.py`, `tests/test_formal_z22_module.py`, `proofs/lean/z22_exclusion.lean` via `z22_no_fixed_point_stabilizer_via_zMap`). The surviving identity cell at `z=(1,0)` is now quantified by exact positive certificates: minimal witness size `6` in full `AGL(2,3)` and `5` in `Hessian216` (`tools/minimal_global_identity_certificates.py`), and this gap is robust under distinct-line and striation-complete witness constraints. Dually, contradiction cores for nontrivial global cells lift from `3` to `4` when striation-complete witnesses are required (`5` at involution `z=(1,0)`), giving a context-complete rigidity profile. In addition, exhaustive nontrivial core-geometry census shows every size-`3` core in `AGL/Hessian` is exactly one full parallel-class triplet, with identical core-signature families across the two spaces (`tools/classify_nontrivial_unsat_core_geometry.py`), and rulebook compression reduces those families to low-complexity coordinate laws with one unique non-cartesian case (`tools/nontrivial_core_rulebook.py`). A fully symbolic, machine-assisted formal proof that removes the finite verification step remains a short-term target; a compact derivation explaining exactly why the allowed `z`-maps are `{(1,0),(2,0),(2,1)}` would be valuable.
