...
if seed_json_path is None and seed_map:
    ...
    open(tmp_seed, "w", encoding="utf-8").write(
        json.dumps({"seed_edges": se, "rotation": None}, indent=2)
    )
...
if seed_json_path is None:
    ...
    tmp_seed = CHECKS / f"_tmp_seed_dd_verify_manual_{int(time.time())}.json"
    open(tmp_seed, "w", encoding="utf-8").write(
        json.dumps({"seed_edges": seed_edges, "rotation": None}, indent=2)
    )
    seed_json_path = tmp_seed
...
if (
    len(art["roots"]) == 2
    and art["roots"][0] is not None
    and art["roots"][1] is not None
):
