...
# try all possible alignments of tomo r3 with axis r_k (k=0,1,2,3)
# r0 is always fixed because tomo r0 == axis r0
r0 = axis_r[0]
results={}
for k in range(4):
    try:
        pi_k = build_pi(r0, tomo_r[3], axis_r[k])
    except Exception as e:
        print(f"alignment with axis r{k} failed: {e}")
        continue
    transported_k = [conjugate_generator(pi_k, tomo_r[i]) for i in range(4)]
    results[k] = {
        'pi': pi_k,
        'transported': transported_k
    }
    print(f"alignment with axis r{k} succeeded, r0 fixed, r3->r{k}")

# save all
json.dump({k:list(results[k]['pi']) for k in results}, open("all_pi_mappings.json","w"), indent=2)
json.dump({k:{f"r{i}":list(results[k]['transported'][i]) for i in range(4)} for k in results}, open("all_transported_r_generators.json","w"), indent=2)
with open("pi_construction_report.txt","w") as f:
    for k in results:
        f.write(f"alignment r3_tomo -> axis r{k}\n")
print("all alignments attempted, outputs saved")
