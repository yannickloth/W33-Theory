...
try:
    ...
    raise SystemExit(
        "This script requires the 'requests' package. Please install it before running (pip install requests)."
    )
...
def ensure_label(repo: str, label: str, token: str):
    ...
    data = {
        "name": label,
        "color": "b60205",
        "description": "Detected W33→E8 embedding found",
    }
...
def issue_exists(
    repo: str, label: str, run_id: int, artifact_name: str, token: str
) -> bool:
    base = f"https://api.github.com/repos/{repo}"
...
def scan_run_for_found(
    repo: str,
    run_id: int,
    run_url: str,
    token: str,
    artifact_prefix: str = "PART_CVII_e8_embedding_sage",
    issue_label: str = "e8-embedding-found",
        ...
        try:
            with zipfile.ZipFile(tmpzip, "r") as z:
                ...
                json_files = [
                    f
                    for f in namelist
                    if f.endswith(".json") and "PART_CVII_e8_embedding_sage" in f
                ]
                if not json_files:
                    ...
                    with z.open(jf) as fh:
                        ...
                        if data.get("found"):
                            ...
                            if not args.no_issue and args.open_issue:
                                ...
                                if not ensured:
                                    print(
                                        f"Warning: could not create or verify label {args.issue_label}"
                                    )
                                # avoid duplicate issues
                                if not issue_exists(
                                    repo, args.issue_label, run_id, name, token
                                ):
                                    ...
                                    try:
                                        url = create_issue(
                                            repo, title, body, [args.issue_label], token
                                        )
                                    ...
                                    print(
                                        "Issue already exists for this artifact; skipping creation."
                                    )
                            # save partial if requested (for found embedding we also save full artifact)
                            if args.save_partial:
                                os.makedirs(
                                    os.path.dirname(args.save_partial), exist_ok=True
                                )
                                from utils.json_safe import dump_json

                                dump_json(data, args.save_partial, indent=2)
                        ...
                        if (
                            best_assigned is not None
                            and best_assigned >= args.partial_threshold
                        ):
                            print(
                                f"Large partial detected in {name}: best_assigned={best_assigned} (run {run_id})"
                            )
                            if args.save_partial:
                                os.makedirs(
                                    os.path.dirname(args.save_partial), exist_ok=True
                            ...
                            if not args.no_issue and args.open_issue:
                                ...
                                if not ensured:
                                    print(
                                        f"Warning: could not create or verify label {args.partial_label}"
                                    )
                                if not issue_exists(
                                    repo, args.partial_label, run_id, name, token
                                ):
                                    title = (
                                        f"W33->E8 large partial found: max_assigned={best_assigned} "
                                        f"(run {run_id}, artifact {name})"
                                    )
                                    body = (
                                        f"A large partial (max_assigned={best_assigned}) was reported by the grid run "
                                        f"[view run]({run_url}) in artifact **{name}**.\n\n"
                                    )
                                    if args.mention:
                                        body += f"cc {args.mention}\n\n"
                                    body += (
                                    ...
                                    print(
                                        "Issue already exists for this partial artifact; skipping creation."
                                    )
                            return True
                # end for json files
        finally:
...
def main(argv: List[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Scan grid run artifacts for embedding 'found: true' or large partials."
    )
    parser.add_argument(
        "--run-id", type=int, required=True, help="Workflow run id to inspect"
    )
    parser.add_argument(
        "--repo",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="URL to the run (for inclusion in issue body)",
    )
    parser.add_argument(
        "--artifact-prefix",
        type=str,
        default="PART_CVII_e8_embedding_sage",
        help="Prefix of artifact names to inspect",
    )
    parser.add_argument(
        "--issue-label",
        type=str,
        default="e8-embedding-found",
        help="Label to apply to created issues when found",
    )
    parser.add_argument(
        "--partial-label",
        type=str,
        default="e8-embedding-partial",
        help="Label to apply to created issues when a large partial is found",
    )
    parser.add_argument(
        "--open-issue",
        action="store_true",
        help="Open a GitHub issue when a 'found: true' is observed",
    )
    parser.add_argument(
        "--partial-threshold",
        type=int,
        default=100,
        help="Trigger a partial alert when best.max_assigned >= threshold",
    )
    parser.add_argument(
        "--save-partial",
        type=str,
        default=None,
        help="Path to save the partial JSON when threshold is exceeded",
    )
    parser.add_argument(
        "--no-issue",
        action="store_true",
        help="Do not open an issue even if a match is detected",
    )
    parser.add_argument(
        "--mention",
        type=str,
        default=os.environ.get("MENTION"),
        help="Optional mention (e.g., @username) to add to issue body",
    )
    ...
    if not token:
        print(
            "Error: GITHUB_TOKEN environment variable is required to call GitHub API."
        )
        return 2
    ...
    found = scan_run_for_found(
        args.repo,
        args.run_id,
        args.run_url or f"https://github.com/{args.repo}/actions/runs/{args.run_id}",
        token,
