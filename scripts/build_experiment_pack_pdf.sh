...
SRC="$REPO_ROOT/docs/experiment_pack.md"
OUT_TEX="$REPO_ROOT/docs/experiment_pack.tex"
OUT_PDF="$REPO_ROOT/docs/experiment_pack.pdf"
TMP_PDF="$REPO_ROOT/docs/experiment_pack.build.pdf"
TMP_MD="$REPO_ROOT/.experiment_pack.build.md"
...
docker run --rm -v "$REPO_ROOT:/data" pandoc/latex \
  -s /data/.experiment_pack.build.md \
  --template=/data/latex/toe_template.tex \
  -V title="Witting/W33 Experiment Pack" \
  -V subtitle="KS Inequality + Z₃ Phase Test" \
  -V date="January 28, 2026" \
  --toc --number-sections \
  -o /data/docs/experiment_pack.tex
...
docker run --rm -v "$REPO_ROOT:/data" pandoc/latex \
  ...
  --toc --number-sections \
  --pdf-engine=pdflatex \
  -o /data/docs/experiment_pack.build.pdf
