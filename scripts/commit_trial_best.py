...
spec_ga = importlib.util.spec_from_file_location('git_auto_keep','scripts/git_auto_keep.py')
...
def main():
    ...
    artifact = {'trial': trial, 'bijection': j.get('bijection'), 'verification': trial.get('verification')}
    outname.parent.mkdir(parents=True, exist_ok=True)
    outname.write_text(json.dumps(artifact, indent=2), encoding='utf-8')
    print('Wrote', outname)
    ok, msg = git_auto_keep.git_add_commit([str(outname)], f'Add trial artifact: {infile.name} best_exact={trial.get("best_exact")}', branch='auto-keep', push=True)
