...
def build_local_model(hot_edges, radius, K):
    ...
    cost = np.linalg.norm(build_edge_vectors(X, edges)[:, None, :] - roots_unit[None, :, :], axis=2)
    ...
    for e in hot_edges:
        ...
        involved_vertices.add(i); involved_vertices.add(j)
    if radius > 0:
        ...
        for _ in range(radius):
            ...
            for ei, (a,b) in edge_to_vertices.items():
                if a in frontier or b in frontier:
                    ...
                    new_frontier.add(a); new_frontier.add(b)
...
def test_forced_pairs(edges, pairs_to_test, radius, K, time_limit, num_workers=8):
    try:
        ...
        err = f"Error importing ortools.cp_model in test_forced_pairs: {exc}\n" + traceback.format_exc()
        print(err, flush=True)
    ...
    for (e, r) in pairs_to_test:
        ...
        try:
            model2, bvars2, candidates2, involved_edges2 = build_local_model(edges, radius, K)
        except Exception:
            results.append({'edge': e, 'root': r, 'status': 'ERROR_BUILD_MODEL', 'error': traceback.format_exc()})
            continue
        ...
        try:
            ...
            results.append({'edge': e, 'root': r, 'status': 'ERROR', 'error': traceback.format_exc()})
    return results
...
def main():
    ...
    parser.add_argument('--check-ortools', action='store_true', help='Only check ortools cp_model import and exit')
    args = parser.parse_args()

    ts = int(time.time())
    out_dir = Path(args.log_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    logfile = out_dir / f'PART_CVII_local_hotspot_feasibility_{"_".join(map(str,args.edges))}_{ts}.log'
    ...
    print(f"Starting local_hotspot_feasibility with edges={args.edges} k={args.k} radius={args.radius} time_limit={args.time_limit} (ts={ts})", flush=True)
    ...
    candidates = {e: list(np.argsort(cost[e])[:args.k]) for e in range(len(edges_list))}

    # report candidate counts for hotspot edges
    for e in args.edges:
        print(f"edge {e} candidates: {len(candidates.get(e, []))}", flush=True)
    if args.dump_candidates:
        cand_file = out_dir / f'PART_CVII_local_hotspot_candidates_{"_".join(map(str,args.edges))}_{ts}.json'
        # ensure indices are native Python ints for JSON serialization
        cand_dump = {str(e): [int(x) for x in candidates.get(e, [])] for e in args.edges}
    ...
    if len(args.edges) == 1:
        try:
            single_results = test_forced_pairs([first_edge], [(first_edge, r) for r in candidates[first_edge]], args.radius, args.k, args.time_limit, num_workers=args.workers)
        ...
        for idx, (e1, r1, e2, r2) in enumerate(pairs, start=1):
            if idx % 50 == 0 or idx == 1:
                print(f'Testing pair {idx}/{total} (edge {e1} root {r1}  vs edge {e2} root {r2})', flush=True)
                logging.info('Testing pair %d/%d: %s', idx, total, (e1, r1, e2, r2))
            try:
                model, bvars, cand, involved = build_local_model(args.edges, args.radius, args.k)
                # ensure both edges are present in model
                if (e1, r1) not in bvars or (e2, r2) not in bvars:
                    results['tests'].append({'pair':[e1,r1,e2,r2], 'status':'NOT_IN_MODEL'})
                    continue
                ...
                try:
                    ...
                    status_name = solver.StatusName(st)
                    results['tests'].append({'pair':[e1,r1,e2,r2], 'status': status_name})
                except Exception:
                    err = traceback.format_exc()
                    print(f'Exception during solver.Solve for pair {e1,r1,e2,r2}: {err}', flush=True)
                    logging.exception('Solver.Solve failed for pair %s', (e1,r1,e2,r2))
                    results['tests'].append({'pair':[e1,r1,e2,r2], 'status':'ERROR_SOLVE', 'error': err})
            except Exception:
                err = traceback.format_exc()
                print(f'Exception building model or testing pair {e1,r1,e2,r2}: {err}', flush=True)
                logging.exception('Error in pair test for %s', (e1,r1,e2,r2))
                results['tests'].append({'pair':[e1,r1,e2,r2], 'status':'ERROR', 'error': err})

    def _make_json_serializable(o):
        # Recursively convert numpy types to Python built-ins suitable for json.dumps
        if isinstance(o, dict):
            return { (str(k) if not isinstance(k, str) else k): _make_json_serializable(v) for k, v in o.items() }
    ...
    try:
        ...
        try:
            ...
            fallback.write_text(json.dumps(_make_json_serializable(results), indent=2), encoding='utf-8')
