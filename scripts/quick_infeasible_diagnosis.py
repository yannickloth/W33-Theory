...
from analyze_infeasible_blocks import (
    load_bijection,
    run_forced_seed,
    shrink_unsat,
    write_seed_for_edges,
)
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=8, help="Number of infeasible blocks to process"
    )
    ...
    for f, j in ins[: args.limit]:
        ...
        if status == "INFEASIBLE":
            min_conflict = shrink_unsat(
                ...
                seed=args.seed,
                max_checks=args.shrink_max_checks,
            )
        ...
        outp_partial = (
            Path.cwd()
            / "checks"
            / f"PART_CVII_infeasible_block_analysis_quick_partial_{stamp_now}.json"
        )
        ...
        if getattr(args, "auto_commit", False):
            try:
                ...
                artifact.write_text(
                    outp_partial.read_text(encoding="utf-8"), encoding="utf-8"
                )
                ok, msg = git_add_commit(
                    [str(artifact)],
                    f"Quick infeasible analysis partial: {outp_partial.name}",
                )
    ...
    outp = (
        Path.cwd()
        / "checks"
        / f"PART_CVII_infeasible_block_analysis_quick_{stamp}.json"
    )
