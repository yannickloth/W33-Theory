...
def collect_pairs_from_files(patterns: List[str]) -> List[Tuple[Tuple[int,int,int,int], str]]:
    pairs = []
    for p in patterns:
        for fname in sorted(glob.glob(p)):
            ...
            for t in tests:
                ...
                if str(status).upper().startswith('INFEASIBLE'):
                    pairs.append(((int(pair[0]), int(pair[1]), int(pair[2]), int(pair[3])), fname))
    return pairs


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--input', type=str, nargs='+', required=True, help='Glob pattern(s) for local_hotspot result files')
    parser.add_argument('--k', type=int, default=40)
    parser.add_argument('--time-limit', type=float, default=30.0)
    parser.add_argument('--seed', type=int, default=212)
    parser.add_argument('--workers', type=int, default=1, help='Run forced checks sequentially by default; >1 runs in parallel (riskier)')
    parser.add_argument('--forbid-json', type=str, default='checks/PART_CVII_forbids.json')
    parser.add_argument('--commit', action='store_true', help='Mirror forbids to committed_artifacts and attempt git commit')
    parser.add_argument('--dry-run', action='store_true', help='List pairs but do not run solver')
    parser.add_argument('--max', type=int, default=0, help='Limit number of pairs to verify; 0 means no limit')
    parser.add_argument('--checks-dir', type=str, default='checks', help='Directory to read/write solver check files')
    ...
    for (e1,r1,e2,r2), src in pairs:
        ...
        if args.dry_run:
            ...
            continue
        res = run_forced_seed(seed_path, k=args.k, time_limit=args.time_limit, seed=args.seed)
        ...
        entry = {
            'pair':[e1,r1,e2,r2],
            'local_source': src,
            'solver_process': {'returncode': res.get('returncode'), 'stdout': res.get('stdout'), 'stderr': res.get('stderr')},
        ...
        if status == 'INFEASIBLE':
            appended = append_forbid(forbid_path, [int(e1), int(e2)], [int(r1), int(r2)], src)
            if appended:
                print('Appended forbids entry for pair', (e1,e2), '-> roots', (r1,r2))
            else:
                print('Forbid entry already present for pair', (e1,e2), '-> roots', (r1,r2))
            ...
            if args.commit:
                ...
                art.write_text(forbid_path.read_text(encoding='utf-8'), encoding='utf-8')
                try:
                    from git_auto_keep import git_add_commit
                    ok, msg = git_add_commit([str(art)], f'Auto-register verified pair forbid: {e1}-{e2}')
                    print('Auto-commit result:', ok, msg)
                except Exception as e:
                    print('Auto-commit failed:', e)
        else:
            print('Solver did not return INFEASIBLE for pair', (e1,r1,e2,r2), 'status=', status)
