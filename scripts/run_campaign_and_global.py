...
def find_latest_seed(pattern='checks/PART_CVII_e8_bijection_seed_campaign_*.json') -> Path | None:
...
def main():
    ...
    # Run campaign
    cmd = [
        'py', '-3', '-X', 'utf8', '-u', 'scripts/run_local_patch_campaign.py',
        '--bij', args.bij,
        '--edge-sizes', args.campaign_edge_sizes,
        '--k', str(args.campaign_k),
        '--time-limit', str(args.campaign_time),
        '--seed', str(args.campaign_seed),
        '--seed-reward', str(args.campaign_seed_reward),
        '--max-iter', str(args.campaign_iter)
    ]
    if args.auto_commit:
        cmd += ['--auto-commit']
    ...
    cp_cmd = [
        ...
        '--seed-json', str(latest_seed),
        '--k', str(args.cp_k),
        '--time-limit', str(args.cp_time),
        '--seed', str(args.cp_seed),
        '--seed-reward', str(args.cp_seed_reward),
        '--log'
    ]
    ...
    if res_path.exists() and args.auto_commit:
        ...
        try:
            ...
            ok, msg = git_auto_keep.git_add_commit([str(artifact)], f'Global CP-SAT result: {artifact.name}', branch=args.commit_branch, push=args.push_commits)
