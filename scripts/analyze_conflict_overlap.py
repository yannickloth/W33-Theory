...
candidates = sorted(Path('checks').glob('PART_CVII_infeasible_block_analysis_quick_merged_*.json'))
if not candidates:
...
for idx, cnt in edge_counts.most_common(20):
    ...
    hotspots.append({'edge_index': idx, 'edge': uv, 'count': cnt, 'blocks': edge_to_blocks[idx]})
...
outp = Path('checks') / f'PART_CVII_conflict_hotspots_{int(__import__("time").time())}.json'
