...
def main():
    ...
    parser.add_argument('--edge-sizes', type=str, default='24', help='Comma-separated list of edge sizes to try, e.g., "24,48,72"')
    ...
    parser.add_argument('--auto-commit', action='store_true', help='Auto-commit intermediate bijection + seed when a patch is applied')
    parser.add_argument('--commit-branch', type=str, default=None, help='Branch name used if push is requested')
    parser.add_argument('--push-commits', action='store_true', help='Push commits to remote when --auto-commit is used')
    parser.add_argument('--check-collaborator', action='store_true', help='Run collaborator activity check at end of each iteration')
    ...
    current_exact = sum(1 for t in tri_list if triangle_exact(roots, current_bij, edges, t))
    print('Initial exact triangles:', current_exact)

    # write initial seed
    seed_file = Path.cwd() / 'checks' / f'PART_CVII_e8_bijection_seed_campaign_{int(time.time())}.json'
    seed_obj = {'seed_edges': [{'edge_index': int(k), 'root_index': int(v)} for k, v in current_bij.items()], 'rotation': None}
    ...
    while iter_no < args.max_iter and improved:
        ...
        for sv in start_vertices:
            ...
            for size in expand_sizes:
                try:
                    run_local(str(seed_file), sv, size, args.k, args.time_limit, args.seed, args.seed_reward)
                ...
                latest = sorted(glob('checks/PART_CVII_e8_bijection_local_seed_*.json'), key=os.path.getmtime)[-1]
                res = json.loads(open(latest, encoding='utf-8').read())
                status = res.get('status')
                if status != 'OPTIMAL' and status != 'FEASIBLE':
                    print('No assignment from local run at vertex', sv, 'size', size, 'status', status)
                    continue
                ...
                # recompute exact
                new_exact = sum(1 for t in tri_list if triangle_exact(roots, current_bij, edges, t))
                delta = new_exact - current_exact
                if delta > 0:
                    print(f'Applied local patch at vertex {sv}, size {size}: delta={delta} ({current_exact}->{new_exact})')
                    ...
                    stamp = int(time.time())
                    outpath = Path.cwd() / 'checks' / f'PART_CVII_e8_bijection_intermediate_{stamp}.json'
                    outpath.write_text(json.dumps({'bijection': {str(k): int(v) for k, v in current_bij.items()}}, indent=2), encoding='utf-8')

                    # Mirror to tracked artifacts/ to avoid git-ignored checks/
                    artifact_out = Path.cwd() / 'committed_artifacts' / outpath.name
                    artifact_out.parent.mkdir(parents=True, exist_ok=True)
                    artifact_out.write_text(json.dumps({'bijection': {str(k): int(v) for k, v in current_bij.items()}}, indent=2), encoding='utf-8')
                    print('Wrote intermediate to', outpath, 'and committed artifact', artifact_out)
                    ...
                    if getattr(args, 'auto_commit', False):
                        try:
                            ...
                            artifact_seed = Path.cwd() / 'committed_artifacts' / seed_file.name
                            artifact_seed.parent.mkdir(parents=True, exist_ok=True)
                            artifact_seed.write_text(seed_file.read_text(encoding='utf-8'), encoding='utf-8')
                            ...
                            ok, msg = git_auto_keep.git_add_commit(files_to_commit, commit_msg, branch=args.commit_branch, push=args.push_commits)
                    ...
                    print(f'Patch at vertex {sv}, size {size} made no improvement (delta={delta})')
            if applied:
        ...
        if getattr(args, 'check_collaborator', False):
            try:
                subprocess.run(['py', '-3', '-X', 'utf8', 'scripts/check_collaborator_edits.py'], check=False)
            except Exception as e:
                print('check_collaborator_edits failed:', e)

    # Final write
    stamp = int(time.time())
    outpath = Path.cwd() / 'checks' / f'PART_CVII_e8_bijection_campaign_result_{stamp}.json'
    out = {'bijection': {str(k): int(v) for k, v in current_bij.items()}, 'final_exact': int(current_exact)}
