...
print(
    f"Found {len(candidates)} small entries (<= {args.threshold}) to run dd_shrink on"
)
...
for f, idx, size in candidates:
    ...
    logp.write_text(
        logp.read_text() + f"Run: {f} idx {idx} size {size} at {time.time()}\n"
    )
    cmd = [
        "py",
        "-3",
    ...
    logp.write_text(
        logp.read_text() + f"Exit code: {ret.returncode} at {time.time()}\n"
    )
    time.sleep(0.2)

print(
    "Sweep done; now running register_dd_obstructions.py to verify and record artifacts"
)
