...
def main():
    ...
    parser.add_argument(
        "--max-size", type=int, default=2, help="Max subset size to test"
    )
    ...
    for s in range(1, min(args.max_size, n) + 1):
        for comb in combinations(range(n), s):
            ...
            json.dump(
                {"obstruction_sets": subset}, open(TMP, "w", encoding="utf-8"), indent=2
            )
            status = run_solver_for_forbids(TMP, args.k, args.time_limit, args.seed)
            results["tested"] += 1
            if status == "INFEASIBLE":
                results["found"].append(
                    {"indices": list(comb), "subset": subset, "status": status}
                )
