...
def main():
    ...
    parser.add_argument(
        "--auto-commit",
        action="store_true",
        help="If set, commit patched bijection to git",
    )
    parser.add_argument(
        "--commit-branch",
        type=str,
    ...
    if getattr(args, "auto_commit", False):
        try:
            ...
            commit_msg = (
                args.commit_msg
                or f"Apply local bijection patch: delta={delta} [{stamp}]"
            )
            ...
            seed_out = (
                Path.cwd()
                / "checks"
                / f"PART_CVII_e8_bijection_seed_patched_{stamp}.json"
            )
            ...
            subprocess.run(
                [
                    ...
                    "-X",
                    "utf8",
                    "scripts/write_seed_from_bijection.py",
                    "--in",
                    str(outpath),
                    "--out",
                    str(seed_out),
            ...
            artifact_seed.write_text(
                seed_out.read_text(encoding="utf-8"), encoding="utf-8"
            )
            ok2, msg2 = git_auto_keep.git_add_commit(
                [str(artifact_seed)],
                f"Seed from patched bijection: {stamp}",
                branch=args.commit_branch,
                push=args.push,
            )
