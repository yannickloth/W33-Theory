...
def main():
    ...
    parser.add_argument('--auto-commit', action='store_true', help='If set, commit seed to git')
    ...
    if getattr(args, 'auto_commit', False):
        try:
            ...
            ok, msg = git_auto_keep.git_add_commit([str(artifact_out)], f'Write seed from bijection: {artifact_out.name}', branch=args.commit_branch, push=args.push)
