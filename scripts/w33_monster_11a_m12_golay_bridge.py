...
def _perm_inverse(perm: tuple[int, ...]) -> tuple[int, ...]:
    inv = [0] * len(perm)
    for i, j in enumerate(perm):
        inv[int(j)] = int(i)
    return tuple(inv)
...
def _monomial_mul(
    ...
    """Multiply monomial maps M = D*P in our coordinate convention."""
    perm_a, signs_a = a
    perm_b, signs_b = b
    n = len(perm_a)
    ...
    inv_perm_a = _perm_inverse(perm_a)
    signs_out = tuple(
...
def _monomial_inv(
    elem: tuple[tuple[int, ...], tuple[int, ...]]
) -> tuple[tuple[int, ...], tuple[int, ...]]:
    perm, signs = elem
    n = len(perm)
    inv_perm = _perm_inverse(perm)
    # (D*P)^{-1} = P^{-1}*D^{-1} = D'*P^{-1} with D' reindexed by P.
    signs_inv = tuple(int(signs[int(perm[j])]) % 3 for j in range(n))
    return inv_perm, signs_inv


def _monomial_group_order(
    gens: list[tuple[tuple[int, ...], tuple[int, ...]]]
) -> int:
    """BFS the monomial subgroup generated by `gens` (expects ~190,080 for 2.M12)."""
    n = len(gens[0][0])
    ident = (tuple(range(n)), tuple([1] * n))
    seen: set[tuple[tuple[int, ...], tuple[int, ...]]] = {ident}
    q: deque[tuple[tuple[int, ...], tuple[int, ...]]] = deque([ident])
    while q:
        x = q.popleft()
        for g in gens:
            y = _monomial_mul(x, g)
...
def analyze(*, compute_monomial_order: bool = False) -> dict[str, Any]:
    ...
    if compute_monomial_order:
        ...
        g2 = (b21_code, signs_21)
        gens = [g1, g2, _monomial_inv(g1), _monomial_inv(g2)]
        monomial_order = int(_monomial_group_order(gens))
...
def main() -> None:
    rep = analyze(compute_monomial_order=True)
    ...
    mg = rep.get("golay", {}).get("monomial_group_order")
    if mg:
        print()
        print("§5. Lifted monomial group")
        print("-" * 58)
        print(f"  generated order = {mg}")
        print(f"  equals 2·|M12|? {mg == 2 * idx['m12_order']}")
