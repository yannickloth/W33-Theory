...
# the sign-lift logic has been moved to monomial_utils.find_sign_lift_for_perm
...
def analyze(*, compute_monomial_order: bool = False) -> dict[str, Any]:
    ...
    # ---------------------------------------------------------------------
    signs_11 = find_sign_lift_for_perm(
        perm=b11_code, generator_rows=generator_rows, code_set=code_set
    )
    signs_21 = find_sign_lift_for_perm(
    ...
    if compute_monomial_order:
        ...
        gens = [g1, g2, monomial_inv(g1), monomial_inv(g2)]
        monomial_order = int(monomial_group_order(gens))
