...
PATTERNS = [
    "committed_PART_CVII_e8_bijection_trial_*.json",
    "committed_PART_CVII_e8_bijection_campaign_best_*.json",
    "PART_CVII_e8_bijection_campaign_best_*.json",
]
...
def save_state(s: dict):
    ...
    dump_json(s, STATE_FILE, indent=2)
...
def best_exact_from_file(path: Path) -> Optional[int]:
    ...
    if isinstance(obj, dict):
        if (
            "trial" in obj
            and isinstance(obj["trial"], dict)
            and "best_exact" in obj["trial"]
        ):
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum improvement in best_exact to trigger repair",
    )
    parser.add_argument(
        "--edge-sizes",
        type=str,
        default="24",
        help="Comma-separated edge sizes to pass to local campaign",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=120.0,
        help="Time limit passed to local solver (seconds)",
    )
    parser.add_argument(
        "--max-iter",
        type=int,
        default=2,
        help="Max iterations for local patch campaign",
    )
    parser.add_argument(
        "--auto-commit",
        action="store_true",
        help="Let local campaign auto-commit intermediate bijections",
    )
    parser.add_argument("--commit-branch", type=str, default="auto-keep")
    parser.add_argument(
        "--push-commits",
        action="store_true",
        help="Push commits made by local campaign",
    )
    args = parser.parse_args()

    watch_dir = Path(args.watch)
    snapshot = (
        {str(p): p.stat().st_mtime for p in watch_dir.glob("*") if p.is_file()}
        if watch_dir.exists()
        else {}
    )
    ...
    try:
        while True:
            ...
            if candidate and candidate_best >= last_best + args.min_delta:
                log(
                    f"New best detected: {candidate} best_exact={candidate_best} (prev={last_best})"
                )
