...
partials = sorted(
    glob.glob("checks/PART_CVII_infeasible_block_analysis_quick_partial_*.json")
)
    ...
    try:
        ...
        for entry in j.get("checked", []):
            ...
            if key not in seen or (
                entry.get("minimal_conflict_size")
                and (
                    seen[key].get("minimal_conflict_size", 1e9)
                    > entry.get("minimal_conflict_size")
                )
            ):
                seen[key] = entry
...
# basic stats
sizes = [
    e.get("minimal_conflict_size")
    for e in merged["checked"]
    if isinstance(e.get("minimal_conflict_size"), int)
]
if sizes:
    sizes.sort()
    stats = {
...
outp = (
    Path("checks")
    / f"PART_CVII_infeasible_block_analysis_quick_merged_{int(time.time())}.json"
)
