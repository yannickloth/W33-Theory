...
def run_solver_with_forbids(forb_path: Path, k: int, time_limit: int, seed: int):
    ...
    log(
        f"Solver exit {proc.returncode} stdout(len)={len(proc.stdout)} stderr(len)={len(proc.stderr)}"
    )
    # read solver JSON
...
def main():
    ...
    parser.add_argument(
        "--commit",
        action="store_true",
        help="Commit minimized forbids to checks/PART_CVII_forbids.json",
    )
    parser.add_argument(
        "--forbid-file",
        type=str,
        default=str(FORB),
        help="Path to forbids JSON to minimize",
    )
    ...
    if status != "INFEASIBLE":
        log(
            f"Full forbids did not prove INFEASIBLE (status={status}). Aborting minimization."
        )
        raise SystemExit(0)
    ...
    while changed:
        ...
        for i, entry in enumerate(list(current)):
            ...
            json.dump(
                {"obstruction_sets": trial}, open(TMP, "w", encoding="utf-8"), indent=2
            )
