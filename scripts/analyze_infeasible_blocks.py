...
def run_forced_seed(seed_json: Path, k: int, time_limit: float, seed: int) -> Dict[str, Any]:
    cmd = [
...
def shrink_unsat(bij: Dict[int, int], edges: List[int], k: int, time_limit: float, seed: int, max_checks: int | None = None) -> List[int]:
    ...
    while changed:
        ...
        for e in list(U):
            ...
            res = run_forced_seed(tmp, k=k, time_limit=max(5, time_limit/6), seed=seed)
            checks_done += 1
...
def main():
    ...
    parser.add_argument('--shrink-max-checks', type=int, default=50,
    ...
    results = {'checked': [], 'timestamp': int(time.time()), 'bij_source': str(bij_path)}
    ...
    for f, j in infeasible_files:
        ...
        if status == 'INFEASIBLE':
            ...
            max_checks = None if args.shrink_max_checks is None or args.shrink_max_checks <= 0 else int(args.shrink_max_checks)
            min_conflict = shrink_unsat(bij, block_edges, k=args.k, time_limit=args.time_limit, seed=args.seed, max_checks=max_checks)
    ...
    if getattr(args, 'auto_commit', False):
        try:
            ...
            ok, msg = git_auto_keep.git_add_commit([str(artifact)], f'Infeasible block analysis: {outp.name}')
