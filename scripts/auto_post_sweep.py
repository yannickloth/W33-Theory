...
def run_pipeline(
    k: int, prove_time: int, pair_time: int, global_time: int, commit: bool, seed: int
):
    ts = int(time.time())
    report = {
        "ts": ts,
    ...
    dump_json(report, outp, indent=2)
    # mirror report
    ...
    # update state
    dump_json({"last_run": time.time(), "report": str(outp)}, STATE, indent=2)
    return report
...
def main():
    ...
    parser.add_argument(
        "--prove-time", type=int, default=300, help="seconds for minimizer prove step"
    )
    parser.add_argument(
        "--pair-time", type=int, default=60, help="seconds per pair test"
    )
    parser.add_argument(
        "--global-time", type=int, default=600, help="seconds for long global prove"
    )
    ...
    log(
        f"Auto-post-sweep started; quiet={args.quiet_minutes}m poll={args.poll_interval}s commit={args.commit}"
    )
    try:
        while True:
            now = time.time()
            ...
            if curr_change != last_change:
                ...
                if idle >= quiet_s and (now - last_run) >= min_interval_s:
                    log(
                        f"Idle for {idle}s >= quiet({quiet_s}s) and last_run {(now - last_run)}s ago; triggering pipeline"
                    )
                    report = run_pipeline(
                        args.k,
                        args.prove_time,
