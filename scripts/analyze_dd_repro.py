...
files = glob.glob(str(CHECKS / "PART_CVII_dd_shrink_result_*.json")) + glob.glob(
    str(ART / "PART_CVII_dd_shrink_result_*.json")
)
summary = {
    ...
    summary["entries"].append(
        {
            "path": p,
            "shrink_status": st,
            "initial_reproducible": ir,
            "seed_source": ss,
            "seed_artifact": j.get("seed_artifact"),
            "result": j.get("result"),
            "time_seconds": ts,
        }
    )
