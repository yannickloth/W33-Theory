...
def test_toe_fermion_mass_structure_from_w33():
    """Validate inter-generation Yukawa/mass data generated by w33_fermion_masses.py."""
    data = _load("artifacts/PART_CVII_fermion_masses.json")

    # Basic structural assertions
    assert data["generation_symmetric"] is False
    assert data["Y_diagonal_uniform"] is False
    assert data["Y_offdiag_uniform"] is False

    # Yukawa eigenvalues (ascending) match the computed result
    expected_eigs = [0.0006894155407016628, 0.010533438104637275, 0.05023635478331946]
    assert [pytest.approx(v, rel=1e-9) for v in data["Y_eigenvalues"]] == [
        pytest.approx(v, rel=1e-9) for v in expected_eigs
    ]

    # Trace consistency: sum of eigenvalues == trace(Y)
    trace = sum(data["Y_matrix"][i][i] for i in range(3))
    assert pytest.approx(sum(data["Y_eigenvalues"]), rel=1e-12) == pytest.approx(
        trace, rel=1e-12
    )
    ...
    for i in range(3):
        for j in range(3):
            assert pytest.approx(Y[i][j], rel=1e-12) == pytest.approx(
                Y[j][i], rel=1e-12
            )
