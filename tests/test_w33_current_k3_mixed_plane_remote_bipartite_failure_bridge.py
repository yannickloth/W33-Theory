...
def test_current_remote_bipartite_failure_has_expected_zero_state() -> None:
    summary = build_current_k3_mixed_plane_remote_bipartite_failure_summary()
    current = summary["current_mixed_plane_remote_bipartite_state"]

    assert current["current_upper_remote_supported_entry_count"] == 0
    assert current["current_lower_remote_supported_entry_count"] == 0


def test_current_remote_bipartite_failure_theorem_holds() -> None:
