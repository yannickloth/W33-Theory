...
def test_w33_heisenberg_universal(tmp_path):
    ...
    files = sorted(
        list(chk_dir.glob("PART_CVII_heisenberg_qutrit_*.json")),
...
def test_parallelogram_holonomy_all_match():
    ...
    assert j.get("total_parallelograms") == j.get(
        "matches"
    ), "Not all parallelograms matched Bargmann phases"
...
def test_phase_propagation_adjusts_all_n12():
    ...
    assert (
        out_json.exists()
    ), "parallelogram comparison JSON missing after phase propagation"
