from exploration.w33_spectral_rosetta_lock_bridge import build_spectral_rosetta_lock_summary


def test_spectral_data_recovers_q_phi3_phi6() -> None:
    summary = build_spectral_rosetta_lock_summary()
    data = summary["spectral_data"]
    assert data["k"] == 12
    assert data["r"] == 2
    assert data["s"] == -4
    assert data["q_from_r_plus_one"] == 3
    assert data["phi3_from_k_plus_one"] == 13
    assert data["phi6_from_one_plus_r_minus_s"] == 7


def test_promoted_observables_match_direct_spectral_formulas() -> None:
    summary = build_spectral_rosetta_lock_summary()
