...
def test_agl23_count_and_commutator():
    ...
    Tx = [int(txj['Tx']['perm_40'][str(i)]) for i in range(40)] if isinstance(txj['Tx']['perm_40'], dict) else txj['Tx']['perm_40']
    Ty = [int(txj['Ty']['perm_40'][str(i)]) for i in range(40)] if isinstance(txj['Ty']['perm_40'], dict) else txj['Ty']['perm_40']
    ...
    expected = [int(Z_perm[str(i)]) for i in range(40)] if isinstance(Z_perm, dict) else Z_perm
