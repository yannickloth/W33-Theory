...
def test_current_mixed_plane_triangle_row_curvature_failure_has_expected_shape() -> None:
    summary = build_current_k3_mixed_plane_triangle_row_curvature_failure_summary()
    state = summary["current_mixed_plane_triangle_row_curvature_state"]
    exact = summary["exact_triangle_row_curvature_witness"]
    ...
    assert state["current_supported_triangle_count"] == 0
    assert state["current_supported_row_count"] == 0

    assert exact["supported_triangle_count"] == 2428
    assert exact["supported_row_count"] == 4046


def test_current_mixed_plane_triangle_row_curvature_failure_theorem_holds() -> None:
