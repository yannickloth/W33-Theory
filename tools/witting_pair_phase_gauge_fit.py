#!/usr/bin/env python3
"""Gauge-fix pairwise phases to fit algebraic forms over GF(3).

We allow per-ray phase shifts s_i (mod 3) so that:
...
def build_pair_list(rays):
    ...
    for i in range(40):
        for j in range(i + 1, 40):
            ...
            if abs(ip) < 1e-8:
                continue
            k = phase_to_k(np.angle(ip)) % 3
            pairs.append((i, j, k))
    return pairs


def build_features_bilinear(xi, xj):
    feats = []
    for a in range(4):
        for b in range(4):
...
def build_features_quadratic(xi, xj):
    v = list(xi) + list(xj)
    feats = []
...
def main():
    print("PAIR-PHASE GAUGE FIT (F3^4)")
    print("=" * 60)
    ...
    pairs = build_pair_list(rays)
    print(f"Non-orth pairs: {len(pairs)}")

    # unknowns: coeffs + s_i for i in 0..39
    # build bilinear system
    X = []
    y = []
    for i, j, k in pairs:
    ...
    if sol is None:
        print("No gauge+bilinear solution (mod 3).")
        bilinear_ok = False
    else:
        bilinear_ok = True
        preds = (X @ sol) % 3
        ok = int(np.sum(preds == y))
        print(f"Gauge+bilinear fit: {ok}/{len(y)} correct")

    # build quadratic system
    Z = []
    z = []
    for i, j, k in pairs:
    ...
    if sol2 is None:
        print("No gauge+quadratic solution (mod 3).")
        quad_ok = False
    else:
        quad_ok = True
        preds = (Z @ sol2) % 3
        ok = int(np.sum(preds == z))
        print(f"Gauge+quadratic fit: {ok}/{len(z)} correct")

    out = {
        "pairs": len(pairs),
        "gauge_bilinear_fit": bool(bilinear_ok),
        "gauge_quadratic_fit": bool(quad_ok),
    }
    out_path = ROOT / "artifacts" / "witting_pair_phase_gauge_fit.json"
