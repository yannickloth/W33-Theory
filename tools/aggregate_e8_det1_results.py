...
def main() -> None:
    ...
    print('Cycles with at least one A2 divisible by 3 (S%3==0):', any_divisible, f"({any_divisible/n:.3%})")
    print('Cycles with at least one A2 where s_mod3 == k:', any_match, f"({any_match/n:.3%})")
    print('Cycles where all 4 A2s are divisible:', all_divisible, f"({all_divisible/n:.3%})")
    ...
    if k_total:
        ...
        print(' - cycles where at least one A2 is divisible:', k_present_and_any_div, f"({k_present_and_any_div/k_total:.3%})")
        print(' - cycles where at least one A2 matches s_mod3 == k:', k_present_and_any_match, f"({k_present_and_any_match/k_total:.3%})")
    ...
    with out.open('w', encoding='utf-8') as f:
        ...
        for i in range(n):
            cyc = all_rows[0][i]['cycle_vertices'] if 'cycle_vertices' in all_rows[0][i] else ','.join(str(x) for x in all_rows[0][i].get('cycle'))
            k = all_rows[0][i].get('k')
            f.write(f"{i},{cyc},{k},{divisible_counts_by_cycle[i] > 0},{match_counts_by_cycle[i] > 0},{divisible_counts_by_cycle[i]},{match_counts_by_cycle[i]}\n")
    print('Wrote', out)
