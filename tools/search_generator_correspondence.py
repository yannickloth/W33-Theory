...
def main():
    ...
    for base_root in bases:
        ...
        print(
            f"base {base_root} best bij {best['bij']} tri_ok {best['tri_ok']} dup {-best['neg_dups']}"
        )
        if best_global is None or (
            best["bij"] * 10000 + best["tri_ok"] * 10 + -best["neg_dups"]
        ) > (
            best_global["bij"] * 10000
            + best_global["tri_ok"] * 10
            + -best_global["neg_dups"]
        ):
            best_global = best
    ...
    (ROOT / "checks" / "PART_CVII_generator_correspondence_search.json").write_text(
    ...
    (ROOT / "artifacts" / "edge_root_bijection_genmap_best.json").write_text(
        json.dumps(map_list, indent=2)
    )
    print(
        "Wrote best results to checks/PART_CVII_generator_correspondence_search.json and artifact mapping"
    )
