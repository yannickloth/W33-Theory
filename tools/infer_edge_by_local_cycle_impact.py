...
def load_candidate_graph(
    path: Path,
) -> Dict[Tuple[int, int], List[Tuple[Tuple[int, ...], str, int]]]:
...
def infer_for_edge(
    ...
    cycles: List[List[int]],
    edge_to_cycles: Dict[Tuple[int, int], List[int]],
    candidate_graph: Dict[Tuple[int, int], List[Tuple[Tuple[int, ...], str, int]]],
    ...
    for vec, tag, score in pool:
        ...
        for idx in cyc_indices:
            ...
            if all_present:
                fixed += 1
        results.append(
            {"vector": list(vec), "tag": tag, "score": score, "fixed_cycles": fixed}
        )
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=3,
        help="If a candidate fixes >= this many cycles, include in a apply CSV",
    )
    p.add_argument(
        "--apply-csv",
        type=Path,
        default=Path(
            "analysis/minimal_commutator_cycles/w33_uv_parser_det1_inferred_apply.csv"
        ),
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not write apply CSV even if candidates meet threshold",
    )
    args = p.parse_args()
                ...
                apply_rows.append(
                    {
                        "edge_a": a,
                        "edge_b": b,
                        "vector": json.dumps(top["vector"]),
                        "score": top["score"],
                        "fixed_cycles": top["fixed_cycles"],
                        "tag": top["tag"],
                    }
                )
