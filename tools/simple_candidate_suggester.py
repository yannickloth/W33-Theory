...
from tools.suggest_missing_edge_candidates import build_inverse_vec_map, build_all_mapped_vectors, dist2
...
OUT = Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_simple_candidates.json')
...
for e in top:
    ...
    if incident:
        ...
        nn = sorted(all_mapped, key=lambda t: dist2(t[0], tuple(int(round(x)) for x in mean)))[:6]
        for vec, edge_pair, tag in nn:
            candlist.append({'vector': list(vec), 'source': f'centroid_nn', 'tag': tag})
    # fallback: global NN to zero
    nn_global = sorted(all_mapped, key=lambda t: dist2(t[0], (0,)*len(all_mapped[0][0])))[:6]
    ...
    for c in candlist:
        ...
        seen.add(vt); final.append(c)

    out['edges'].append({'edge': f'{a},{b}', 'count': e.get('count', 1), 'candidates': final[:10]})
