...
DEFAULT_IN = Path(
    "analysis/minimal_commutator_cycles/e8_det1_combined_aggregated_summary.csv"
)
DEFAULT_OUT_JSON = Path(
    "analysis/minimal_commutator_cycles/problem_cycle_edge_tally.json"
)
DEFAULT_OUT_CSV = Path(
    "analysis/minimal_commutator_cycles/problem_cycle_edge_tally.csv"
)
...
def main() -> None:
    ...
    ap.add_argument(
        "--out-json", type=Path, default=DEFAULT_OUT_JSON, help="Output JSON path"
    )
    ap.add_argument(
        "--out-csv", type=Path, default=DEFAULT_OUT_CSV, help="Output CSV path"
    )
    ...
    out_json.write_text(
        json.dumps(
            {
                "num_problem_cycles": len(problem_cycles),
                "top_edges": [
                    {"edge_a": a, "edge_b": b, "count": c} for (a, b), c in top
                ],
            },
            indent=2,
        ),
    ...
    print(
        "Found",
        len(problem_cycles),
        "problem cycles; wrote top edges to",
        out_json,
        out_csv,
    )
