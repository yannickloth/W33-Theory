#!/usr/bin/env python3
"""Verify CP‑SAT candidates produced by `tools/seed_from_klein.py`.
...
RES_FILE = ROOT / "artifacts" / "klein_seeded_hybrid_results.json"
if not RES_FILE.exists():
    raise SystemExit("No klein_seeded_hybrid_results.json artifact found")
...
# load toe and build br_l2/br_fibers (same as hybrid/seed helpers)
...
# derive bad9_set from canonical artifact
bad9_set = set(
    tuple(sorted(t[:3]))
    for t in json.loads(
        (
...
for rec in data.get("results", []):
    ...
    D = rec.get("D")
    # skip trivial all-zero numerators
    if all(int(n) == 0 for n in nums):
        continue
    support = rec["support"]
    # build full 9-vector candidate (default 1/9)
    cand = [1.0 / 9.0] * 9
