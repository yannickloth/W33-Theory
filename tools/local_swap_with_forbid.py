#!/usr/bin/env python3
"""
Local swap + SA improvement starting from sign-consistent mappings, with the option
to forbid specific E6 triads from being matched (e.g., '--forbid 0-18-25,0-20-23').
Writes per-W results to artifacts/local_swap_with_forbid_W{widx}_results.json and a summary
artifacts/local_swap_with_forbid_summary.json.
"""
...
parser.add_argument(
    ...
    type=str,
    default="",
    help="Comma separated E6 triads like '0-18-25,0-20-23' to forbid",
)
...
if args.forbid:
    for tstr in args.forbid.split(","):
        parts = tuple(sorted(int(x) for x in tstr.strip().split("-")))
        if parts not in E6_TRIADS:
            raise SystemExit(f"forbid tri {parts} not a valid E6 triad")
        FORBID_SET.add(parts)
...
# helper: matched triads count and list for a mapping (E6->coset mapping), and forbidden check
def matched_tris_and_forbidden(mapping, coset_triads_set):
    matched = []
    ...
    for tri in E6_TRIADS:
        ...
        if ct in coset_triads_set:
            if tri in FORBID_SET:
                forbidden_found = True
            else:
                matched.append(tri)
    return matched, forbidden_found
...
for widx in selected:
    ...
    initial_matched_list, initial_forbid = matched_tris_and_forbidden(
        mapping, coset_triads_set
    )
    ...
    best_forbid_free = not initial_forbid
    ...
    while improved and greedy_pass < args.greedy_rounds:
        ...
        for i in range(27):
            ...
            for j in range(i + 1, 27):
                m2 = mapping.copy()
                m2[i], m2[j] = m2[j], m2[i]
                matched2, forb2 = matched_tris_and_forbidden(m2, coset_triads_set)
                if forb2:
                    continue
                if args.preserve_hard and not solvable_sign_system(matched2):
                    continue
                c2 = len(matched2)
                if c2 > best_matched or (c2 == best_matched and (not best_forbid_free)):
                    mapping = m2
                    best_matched = c2
                    best_mapping = m2.copy()
                    best_solvable = solvable_sign_system(matched2)
                    best_forbid_free = True
                    improved = True
    ...
    def score_map(perm):
        matched, forb = matched_tris_and_forbidden(perm, coset_triads_set)
        if forb:
            return -(10**9), 0, False, True
        c = len(matched)
        solv = solvable_sign_system(matched)
        score = c * 1000 - (0 if solv else 100000)
        return score, c, solv, False

    s_curr, c_curr, solv_curr, forb_curr = score_map(perm)
    best_sa = {
        "perm": perm.copy(),
        "score": s_curr,
        "matched": c_curr,
    ...
    if iters > 0:
        ...
        for it in range(iters):
            ...
            s2, c2, solv2, forb2 = score_map(perm2)
            if args.preserve_hard and (not solv2 or forb2):
                accept = False
            else:
                delta = s2 - s_curr
                accept = (delta > 0) or (
            ...
            if accept:
                ...
                if not forb2 and (
                    ...
                    print(
                        f"  SA new best (it {it}) matched {c2} solv {solv2} forb {forb2}"
                    )
            T = max(T * alpha, T_min)
    ...
    out = {
        "W_idx": widx,
        "forbid": [list(t) for t in sorted(FORBID_SET)],
...
# write summary
(ROOT / "artifacts" / "local_swap_with_forbid_summary.json").write_text(
    json.dumps(results, indent=2), encoding="utf-8"
)
print("\nAll done. Summary written to artifacts/local_swap_with_forbid_summary.json")
