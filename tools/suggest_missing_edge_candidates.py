...
def build_inverse_vec_map(
    p: W33RootwordParser,
) -> Dict[Tuple[int, int], List[Tuple[Tuple[int, ...], str]]]:
    """Build edge -> candidate vectors mapping from parser.vec_to_edges_map."""
    edge_map: Dict[Tuple[int, int], List[Tuple[Tuple[int, ...], str]]] = defaultdict(
        list
    )
...
def get_incident_vectors(
    edge_to_root: Dict[Tuple[int, int], Tuple[int, ...]], vertex: int
) -> List[Tuple[int, ...]]:
    out = []
...
def propose_candidates_for_edge(
    a: int,
    b: int,
    p: W33RootwordParser,
    edge_candidates_map: Dict[Tuple[int, int], List[Tuple[Tuple[int, ...], str]]],
    existing_edge_to_root: Dict[Tuple[int, int], Tuple[int, ...]],
    all_mapped_vectors: List[Tuple[Tuple[int, ...], Tuple[int, int], str]],
    candidates_per_edge: int = 5,
) -> List[Dict[str, Any]]:
    ...
    if incident:
        ...
        if cent in getattr(p, "vec_to_edges_map", {}):
            ...
            nn = sorted(all_mapped_vectors, key=lambda t: dist2(t[0], cent))[
                :candidates_per_edge
            ]
            rank = 1
    ...
    if not incident:
        ...
        nn = sorted(all_mapped_vectors, key=lambda t: dist2(t[0], (0,) * len(t[0])))[
            :candidates_per_edge
        ]
        for vec, mapped_edge, tag in nn:
    ...
    # Format sorted list
    final = sorted(by_vec.values(), key=lambda d: d["score"], reverse=True)[
        :candidates_per_edge
    ]
...
def build_all_mapped_vectors(
    p: W33RootwordParser,
) -> List[Tuple[Tuple[int, ...], Tuple[int, int], str]]:
    out = []
...
def generate_report(
    ...
    top_n: int = 40,
    candidates_per_edge: int = 5,
    out_dir: Path = Path("analysis/minimal_commutator_cycles"),
) -> Dict[str, Any]:
    p = W33RootwordParser()
    ...
    for a, b, count in missing_sorted:
        cands = propose_candidates_for_edge(
            a,
            b,
            p,
            edge_candidates_map,
            existing_edge_to_root,
            all_mapped,
            candidates_per_edge=candidates_per_edge,
        )
        report["edges"].append(
    ...
    for ent in report["edges"][: min(20, len(report["edges"]))]:
        top = ent["candidates"][0] if ent["candidates"] else None
        print(
            f"Edge {ent['edge']} (count={ent['count']}): direct={ent['num_direct_candidates']}, top={top['vector'] if top else None} score={top['score'] if top else None} conf={top['confidence'] if top else None}"
        )
...
def main():
    ...
    ap.add_argument(
        ...
        type=Path,
        default=Path(
            "analysis/minimal_commutator_cycles/w33_uv_parser_det1_missing_edges.json"
        ),
    )
    ...
    report = generate_report(
        args.missing_json,
        top_n=args.top_n,
        candidates_per_edge=args.candidates_per_edge,
