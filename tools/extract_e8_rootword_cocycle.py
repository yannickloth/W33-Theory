...
def main():
    ...
    p.add_argument('--cycles-json', type=Path, default=Path('analysis/minimal_commutator_cycles/minimal_holonomy_cycles_ordered_rootwords.json'))
    p.add_argument('--edge-root-json', type=Path, default=Path('artifacts/edge_to_e8_root_combined.json'))
    ...
    p.add_argument('--minimal-cycles-json', type=Path, default=Path('analysis/minimal_commutator_cycles/minimal_holonomy_cycles_ordered_rootwords.json'))
    p.add_argument('--out-dir', type=Path, default=Path('analysis/minimal_commutator_cycles'))
    args = p.parse_args()
    ...
    if isinstance(loaded_cycles, dict):
        if 'canonical_cycles' in loaded_cycles:
            ...
            raise RuntimeError('Unsupported cycles JSON structure; expected list or dict with canonical_cycles key')
    else:
    ...
    if not root_index_map:
        ...
        for ent in json.loads(Path('artifacts/edge_root_bijection_canonical.json').read_text(encoding='utf-8')):
            idx = int(ent['root_index'])
            coords = tuple(int(x) for x in ent['root_coords'])
            if idx not in root_index_map:
                root_index_map[idx] = coords
        ...
        try:
            ...
            for ent in minimal_entries:
                if 'cycle_vertices' in ent and isinstance(ent['cycle_vertices'], str):
                    cyc_list = [int(x) for x in ent['cycle_vertices'].split(',') if x.strip() != '']
    ...
    for c in cycles:
        ...
        if missing:
            rows.append({'id': c.get('id') if isinstance(c, dict) else None, 'k': k_val, 'cycle_vertices': ','.join(str(x) for x in cyc), 'divisible': False, 'reason': 'missing_edge_root', 'missing_edge': f"{missing_edge[0]},{missing_edge[1]}" if missing_edge else None})
            stats['missing_edge_root'] += 1
            continue

        # compute t_i = (r_i,alpha) - (r_i,beta)
        tvals = [inner_prod_doubled(r, alpha) - inner_prod_doubled(r, beta) for r in rlist]
        S = sum(tvals)
        divisible = (S % 3 == 0)
        s_div3 = (S // 3) % 3 if divisible else None
        rows.append({'id': c.get('id') if isinstance(c, dict) else None, 'k': k_val, 'cycle_vertices': ','.join(str(x) for x in cyc), 'S': int(S), 'divisible': bool(divisible), 's_mod3': int(s_div3) if s_div3 is not None else None})
        stats['total'] += 1
        if divisible:
    ...
    divisible_with_k = sum(1 for r in rows if (r.get('k') is not None and r.get('divisible')))
    match_divisible = sum(1 for r in rows if (r.get('k') is not None and r.get('divisible') and r.get('s_mod3') == r.get('k')))

    stats['k_total'] = total_with_k
    stats['divisible_with_k'] = divisible_with_k
    stats['k_match_divisible'] = match_divisible
    stats['match_rate_divisible'] = (match_divisible / divisible_with_k) if divisible_with_k else None

    out = {'a2_index': args.a2_index, 'a2_indices': a2_idxs, 'ai_idx': ai_idx, 'bi_idx': bi_idx, 'rows': rows, 'stats': dict(stats)}
    args.out_dir.mkdir(parents=True, exist_ok=True)
    (args.out_dir / 'e8_rootword_cocycle.json').write_text(json.dumps(out, indent=2), encoding='utf-8')

    # CSV
    with (args.out_dir / 'e8_rootword_cocycle.csv').open('w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['id','k','cycle_vertices','S','divisible','s_mod3'])
        for r in rows:
            writer.writerow([r.get('id'), r.get('k'), r.get('cycle_vertices'), r.get('S'), r.get('divisible'), r.get('s_mod3')])
