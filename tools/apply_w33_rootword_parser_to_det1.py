...
def main():
    ...
    for idx, c in enumerate(cycles):
        ...
        if missing_edge:
            ...
            rows.append({'idx': idx, 'cycle': cyc, 'status': 'missing_edge_root', 'missing_edge': missing_edge_pair})
            continue
        try:
            out = p.parse(rootword)
            k = out['k_canonical']
            rows.append({'idx': idx, 'cycle': cyc, 'status': 'ok', 'k_canonical': int(k), 'n12_pair': out['n12_pair'], 'u_canonical': out['u_canonical'], 'v_canonical': out['v_canonical']})
            stats['ok'] += 1
            stats[f'k_{k}'] += 1
        except Exception as e:
            stats['parse_error'] += 1
            rows.append({'idx': idx, 'cycle': cyc, 'status': 'parse_error', 'error': str(e)})

    out_json = out_dir / 'w33_uv_parser_det1_results.json'
    out_csv = out_dir / 'w33_uv_parser_det1_results.csv'

    out_json.write_text(json.dumps({'rows': rows, 'stats': dict(stats)}, indent=2), encoding='utf-8')

    with out_csv.open('w', encoding='utf-8', newline='') as f:
        w = csv.writer(f)
        w.writerow(['idx', 'status', 'k_canonical', 'n12_pair', 'u_canonical', 'v_canonical', 'cycle'])
        for r in rows:
            w.writerow([r.get('idx'), r.get('status'), r.get('k_canonical'), r.get('n12_pair'), r.get('u_canonical'), r.get('v_canonical'), ','.join(str(x) for x in r.get('cycle'))])
    ...
    if missing_edge_counter:
        ...
        miss_data = [{'edge': f"{a},{b}", 'count': c} for (a, b), c in missing_edge_counter.items()]
        miss_json.write_text(json.dumps({'missing_edges': miss_data}, indent=2), encoding='utf-8')
