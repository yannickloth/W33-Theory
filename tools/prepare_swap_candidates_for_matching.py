...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=DEFAULT_IN,
        help="Swap candidates JSON to convert",
    )
    ap.add_argument(
        "--out-json",
        type=Path,
        default=DEFAULT_OUT_JSON,
        help="Output candidates JSON (matching format)",
    )
    ap.add_argument(
        "--out-sum", type=Path, default=DEFAULT_OUT_SUM, help="Output summary CSV"
    )
    ap.add_argument(
        "--score-mode",
        choices=["fixed", "net"],
        default="fixed",
        help="Which score to emit (default: fixed)",
    )
    args = ap.parse_args()
    ...
    for ent in j.get("out_rows", []):
        ...
        for c in cands:
            ...
            new_cands.append(
                {"vector": vec, "score": score, "tag": "swap-canon", "source": "swap"}
            )
            ...
            rows.append(
                {
                    ...
                    "top_net_matches": int(
                        top.get("net_matches", top.get("fixed_matches", 0))
                    ),
