#!/usr/bin/env python3
"""Point-orbit composition for each line orbit."""
...
OUT_JSON = ROOT / "artifacts" / "witting_pg32_line_orbit_point_composition.json"
OUT_MD = ROOT / "artifacts" / "witting_pg32_line_orbit_point_composition.md"
...
def main():
    ...
    # Map point -> orbit index
    point_orbit_idx = {}
    for i, orb in enumerate(point_orbits):
        for p in orb:
            point_orbit_idx[p] = i
    ...
    # Composition counts
    line_orbit_composition = []
    for orb in line_orbits:
        comps = set()
        for line in orb:
            counts = [0] * len(point_orbits)
            for p in line:
                counts[point_orbit_idx[p]] += 1
            comps.add(tuple(counts))
        line_orbit_composition.append(sorted(comps))

    results = {
        "point_orbit_sizes": [len(o) for o in point_orbits],
        "line_orbit_sizes": [len(o) for o in line_orbits],
        "line_orbit_composition": line_orbit_composition,
    }
    ...
    lines = []
    lines.append("# Line Orbit Point-Compositions")
    lines.append("")
    lines.append(f"- point orbit sizes: {results['point_orbit_sizes']}")
    lines.append(f"- line orbit sizes: {results['line_orbit_sizes']}")
    lines.append("- line orbit compositions (counts in each point-orbit):")
    for i, comps in enumerate(line_orbit_composition):
        lines.append(f"  - orbit {i}: {comps}")
