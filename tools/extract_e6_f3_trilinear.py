...
def annotate_triads(
    triads: List[Dict[str, Any]], labels: Dict[int, HeisenbergLabel]
) -> List[Dict[str, Any]]:
    out = []
    for ent in triads:
...
def main(argv=None):
    ...
    p.add_argument(
        "--out-json", type=Path, default=Path("analysis/e6_f3_trilinear_annotated.json")
    )
    ...
    args.out_json.write_text(
        json.dumps({"annotated_triads": annotated}, indent=2), encoding="utf-8"
    )
