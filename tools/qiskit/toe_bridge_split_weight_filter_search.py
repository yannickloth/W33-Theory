#!/usr/bin/env python3
"""Grover-style search with separate hyperbolic and exceptional weight filters.

This refines the joint weight-filter oracle by splitting the exact packet
concentration theorem into two independent binary factors:

- hyperbolic dominance pass/fail
- exceptional dominance pass/fail

The theorem-backed marked sector keeps only the exact current pass/pass state:
...
HYPERBOLIC_PASS = "hyperbolic_dominance_pass"
HYPERBOLIC_FAIL = "hyperbolic_dominance_fail"
EXCEPTIONAL_PASS = "exceptional_dominance_pass"
EXCEPTIONAL_FAIL = "exceptional_dominance_fail"
...
def build_split_weight_filter_states() -> tuple[
    list[tuple[str, ...]],
    list[tuple[str, ...]],
...
def build_marked_indices(
    ...
    marked_indices = {
        idx
        for idx, (support_idx, factor_idx, glue_state, line_state, hyperbolic_state, exceptional_state) in enumerate(product_states)
        if support_idx in support_marks
        and factor_idx in factor_marks
        and glue_state == glue_target
        and line_state == HEAD_LINE
        and hyperbolic_state == HYPERBOLIC_PASS
        and exceptional_state == EXCEPTIONAL_PASS
    }
    return support_permutations, factor_permutations, product_states, marked_indices


def decode_split_weight_filter_states(
    counts: Counter[str],
    support_permutations: list[tuple[str, ...]],
    factor_permutations: list[tuple[str, ...]],
    product_states: list[tuple[int, int, str, str, str, str]],
    ...
    for bitstring, count in counts.most_common(top):
        ...
        support_idx, factor_idx, glue_state, line_state, hyperbolic_state, exceptional_state = product_states[index]
        ...
        payload["hyperbolic_weight_state"] = hyperbolic_state
        payload["exceptional_weight_state"] = exceptional_state
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the exact TOE bridge split-weight Grover search.")
    parser.add_argument(
        "--mode",
...
def main() -> None:
    ...
    decoded_target, decoded_nontarget_valid, decoded_invalid = decode_split_weight_filter_states(
        counts,
    ...
    summary = {
        "search_name": "toe_bridge_split_weight_filter_search",
        "mode": args.mode,
