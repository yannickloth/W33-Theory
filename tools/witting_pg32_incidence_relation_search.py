#!/usr/bin/env python3
"""Test PG(3,2) incidence relations as a model for W33 adjacency.

Each Witting ray maps via trace images to either:
  - a PG point (size-1 image), or
  - a PG line (size-3 image).

We classify ray pairs by (type, relation) and test all unions of these
classes for exact W33 adjacency match.
...
OUT_JSON = ROOT / "artifacts" / "witting_pg32_incidence_relation_search.json"
OUT_MD = ROOT / "artifacts" / "witting_pg32_incidence_relation_search.md"
...
def main():
    ...
    for s in base_states:
        ...
        if len(imgs) == 1:
            ray_type.append("P")
            ray_obj.append(next(iter(imgs)))
        elif len(imgs) == 3 and tuple(sorted(imgs)) in pg_line_set:
            ray_type.append("L")
            ray_obj.append(tuple(sorted(imgs)))
        else:
            ray_type.append("U")
            ray_obj.append(tuple(sorted(imgs)))
    ...
    for i in range(n):
        for j in range(i + 1, n):
            t1, t2 = ray_type[i], ray_type[j]
            o1, o2 = ray_obj[i], ray_obj[j]
            if t1 == "P" and t2 == "P":
                rel = "PP"
            elif t1 == "P" and t2 == "L":
                rel = "PL_inc" if o1 in o2 else "PL_dis"
            elif t1 == "L" and t2 == "P":
                rel = "PL_inc" if o2 in o1 else "PL_dis"
            elif t1 == "L" and t2 == "L":
                if o1 == o2:
                    rel = "LL_same"
                else:
                    inter = len(set(o1) & set(o2))
                    rel = f"LL_inter{inter}"
            else:
                rel = "U"
            key = rel
    ...
    for mask in range(1, 1 << len(class_keys)):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                in_sel = pair_class[(i, j)] in selected
            ...
            if not ok:
                break
        if ok:
            matches.append(sorted(list(selected)))

    summary = {
        "class_keys": class_keys,
        "class_sizes": {k: len(class_map[k]) for k in class_keys},
        "w33_matches": matches,
    }
    ...
    lines = []
    lines.append("# Witting PG(3,2) Incidence Relation Search")
    lines.append("")
    lines.append(f"- classes: {class_keys}")
    lines.append("## Class sizes")
    for k in class_keys:
        lines.append(f"- {k}: {summary['class_sizes'][k]}")
    lines.append("")
