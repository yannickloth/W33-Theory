...
def main():
    top_edges_j = load_json(
        Path("analysis/minimal_commutator_cycles/problem_cycle_edge_tally.json")
    )
    ...
    cycles_j = load_json(
        Path("analysis/minimal_commutator_cycles/det1_orbit_cycles.json")
    )
        ...
        for r in neighbors[:12]:
            ...
            for idx in cyc_indices:
                ...
                if all_present:
                    fixed += 1
            candidates.append(
                {"vector": list(r), "score": -dist2(r, centroid), "fixed_cycles": fixed}
            )
            if fixed >= 2:
                apply_rows.append(
                    {
    ...
    print(
        "Wrote centroid nearest canonical candidates JSON/CSV and apply CSV (suggest-only)"
    )
