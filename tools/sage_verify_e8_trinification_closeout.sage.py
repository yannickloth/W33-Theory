...
# This file was *autogenerated* from the file tools/sage_verify_e8_trinification_closeout.sage
...
from sage.all import ZZ, CartanType, WeylGroup, matrix  # type: ignore
...
def main() -> None:
    ...
    C_perm = (
        C_raw.matrix_from_rows_and_columns(perm_to_sage, perm_to_sage)
        if ok_matches
        else C_raw
    )
    ...
    we8_order = int(WeylGroup(CartanType(["E", _sage_const_8])).order())

    # Sanity checks expected for E8
    ok = bool(
        ok_matches
        and det_e8 == _sage_const_1
        and det_perm == _sage_const_1
        and we8_order == _sage_const_696729600
    )
    ...
    md.append(
        f"- det(Cartan): raw `{det_raw}`, perm `{det_perm}`, canonical `{det_e8}`"
    )
    ...
    print(json.dumps(out, indent=_sage_const_2, sort_keys=True))
    if not ok:
