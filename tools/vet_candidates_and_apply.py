...
# default files
DEFAULT_CAND_JSON = Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_missing_edges_candidates.json')
DEFAULT_VET_CSV = Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_missing_edges_candidates_vetting.csv')
...
def generate_vetting_csv(cand_json_path: Path, out_csv: Path, score_threshold: int = 80) -> Dict[str, Any]:
    ...
    for ent in edges:
        ...
        for idx, cand in enumerate(ent.get('candidates', [])):
            ...
            suggested = 'yes' if (confidence in ('high', 'medium') or score >= score_threshold) else 'no'
            rows.append({
                'edge_a': a,
                'edge_b': b,
    ...
    with out_csv.open('w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['edge_a', 'edge_b', 'count', 'candidate_idx', 'vector', 'score', 'confidence', 'tag', 'source', 'note', 'derived_from', 'suggested_apply', 'apply', 'comment'])
        for r in rows:
            writer.writerow([r['edge_a'], r['edge_b'], r['count'], r['candidate_idx'], r['vector'], r['score'], r['confidence'], r['tag'], r['source'], r['note'], r['derived_from'], r['suggested_apply'], r['apply'], r['comment']])
    ...
    with csv_path.open('r', encoding='utf-8') as f:
        ...
        for r in reader:
            out.append({
                ...
                'suggested_apply': r.get('suggested_apply', 'no').strip().lower() in ('1', 'yes', 'y', 'true'),
                'apply': r.get('apply', '').strip().lower() in ('1', 'yes', 'y', 'true'),
                'raw_row': r,
            })
    return out
...
def build_consensus_report(cand_json_path: Path, top_n: int = 40, min_occurrences: int = 3, out_csv: Path | None = None) -> Dict[str, Any]:
    ...
    for ent in edges:
        ...
        rec = tally.setdefault(vec, {'occurrences': 0, 'total_count': 0, 'scores': [], 'edges': []})
        rec['occurrences'] += 1
        rec['total_count'] += count
        rec['scores'].append(score)
        rec['edges'].append({'edge': f'{a},{b}', 'count': count, 'score': score, 'tag': top.get('tag'), 'source': top.get('source')})
    ...
    for vec, info in tally.items():
        ...
        rows.append({'vector': list(vec), 'occurrences': occ, 'total_count': info['total_count'], 'avg_score': avg, 'example_edges': [e['edge'] for e in info['edges'][:10]]})
    ...
    if out_csv:
        ...
        with out_csv.open('w', encoding='utf-8', newline='') as f:
            w = csv.writer(f)
            w.writerow(['vector', 'occurrences', 'total_count', 'avg_score', 'example_edges'])
            for r in rows:
                w.writerow([json.dumps(r['vector']), r['occurrences'], r['total_count'], r['avg_score'], json.dumps(r['example_edges'])])
        print('Wrote consensus CSV to', out_csv)

    return {'rows': rows, 'count': len(rows)}


def solve_greedy_matching(cand_json_path: Path, top_n: int = 40, candidates_per_edge: int = 3, priority_bonus: Dict[str, int] | None = None) -> Dict[str, Any]:
    ...
    for score_adj, a, b, vec, tag, source, score in pairs:
        ...
        assignments.append({'edge_a': a, 'edge_b': b, 'vector': list(vec), 'score': score, 'score_adj': score_adj, 'tag': tag, 'source': source})

    stats = {'assigned': len(assignments), 'total_edges': len(edges), 'unique_vectors': len(assigned_vectors)}
    return {'assignments': assignments, 'stats': stats}


def apply_candidates_from_csv(csv_path: Path, combined_map_path: Path = COMBINED_MAP, dry_run: bool = True, force: bool = False) -> Dict[str, Any]:
    ...
    to_apply = [c for c in candidates if c['apply'] or (c['suggested_apply'] and not c['apply'])]
    if not to_apply:
        return {'applied': 0, 'skipped': 0, 'reason': 'no candidates marked for apply'}

    # load combined map
    if not combined_map_path.exists():
        raise FileNotFoundError(f"Combined edge->root file not found: {combined_map_path}")
    combined = json.loads(combined_map_path.read_text(encoding='utf-8'))

    # prepare actions
    actions = []
    for ent in to_apply:
        a = ent['edge_a']; b = ent['edge_b']; vec = ent['vector']
        key = f"({a}, {b})"
        rev_key = f"({b}, {a})"
    ...
    if dry_run:
        summary = {'planned_add': sum(1 for a in actions if a[0] == 'add'), 'planned_skip': sum(1 for a in actions if a[0] != 'add')}
    ...
    print(f'Applied {applied} candidate mappings to {combined_map_path} (skipped {skipped})')
...
def main(argv=None):
    ...
    p.add_argument('--apply-from-csv', type=Path, help='CSV path to read manual apply marks from; if specified, will use its apply column to decide which candidates to apply')
    p.add_argument('--apply', action='store_true', help='Apply proposed candidates (or those marked in CSV)')
    p.add_argument('--dry-run', action='store_true', help='When applying, do not write changes; just report')
    p.add_argument('--force', action='store_true', help='Force overwrite existing mapping entries')

    # consensus options
    p.add_argument('--consensus', action='store_true', help='Build a consensus report for top candidate vectors across missing edges')
    p.add_argument('--consensus-topn', type=int, default=40, help='Top N missing edges to include in consensus analysis')
    p.add_argument('--consensus-min-occurrences', type=int, default=3, help='Minimum occurrences for a candidate vector to appear in consensus report')
    p.add_argument('--consensus-out-csv', type=Path, default=Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_missing_edges_candidates_consensus.csv'))
    p.add_argument('--apply-consensus', action='store_true', help='Apply consensus suggestions (will generate a temp CSV and apply rows marked as suggested)')
    p.add_argument('--consensus-apply-min-avg', type=float, default=0.0, help='Minimum average score for consensus vectors to be considered for apply')

    # greedy matching options
    p.add_argument('--match-greedy', action='store_true', help='Run greedy matching across candidates to propose unique assignments')
    p.add_argument('--match-topn', type=int, default=40, help='Top N missing edges to include in matching')
    p.add_argument('--match-k', type=int, default=3, help='Top K candidates per edge to consider in matching')
    p.add_argument('--match-out-csv', type=Path, default=Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_candidate_matching.csv'))
    p.add_argument('--match-apply', action='store_true', help='Apply matching assignments (generate CSV and use apply-from-csv semantics)')
    ...
    if args.generate_csv or (not args.apply and not args.consensus):
        ...
        gen = generate_vetting_csv(args.candidates_json, args.vet_csv, score_threshold=args.score_threshold)
        print('Generated:', gen)

    if args.consensus:
        print('Building consensus report...')
        cons = build_consensus_report(args.candidates_json, top_n=args.consensus_topn, min_occurrences=args.consensus_min_occurrences, out_csv=args.consensus_out_csv)
    ...
    if args.apply_consensus:
        ...
        cons = build_consensus_report(args.candidates_json, top_n=args.consensus_topn, min_occurrences=args.consensus_min_occurrences)
        chosen = [r for r in cons['rows'] if r['avg_score'] >= args.consensus_apply_min_avg]
        if not chosen:
            ...
            if not tmp_rows:
                ...
                # write tmp csv
                tmp_csv = Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_consensus_apply_tmp.csv')
                with tmp_csv.open('w', encoding='utf-8', newline='') as f:
                    ...
                    for r in tmp_rows:
                        w.writerow([r['edge_a'], r['edge_b'], r['count'], r['candidate_idx'], r['vector'], r['score'], r['confidence'], r['tag'], r['source'], r['note'], r['derived_from'], r['suggested_apply'], r['apply'], r['comment']])
                print('Generated temporary consensus apply CSV:', tmp_csv)
                if args.dry_run:
                    print('Dry-run: not applying changes')
                else:
                    res = apply_candidates_from_csv(tmp_csv, combined_map_path=COMBINED_MAP, dry_run=args.dry_run, force=args.force)
                    print('Apply result:', res)

    if args.match_greedy:
        print('Running greedy matching...')
        match = solve_greedy_matching(args.candidates_json, top_n=args.match_topn, candidates_per_edge=args.match_k, priority_bonus={'canon': 30, 'raw': 10, 'we6': 5, 'e8root': 5})
        ...
        with out.open('w', encoding='utf-8', newline='') as f:
            w = csv.writer(f)
            w.writerow(['edge_a', 'edge_b', 'vector', 'score', 'score_adj', 'tag', 'source'])
            for a in match['assignments']:
                w.writerow([a['edge_a'], a['edge_b'], json.dumps(a['vector']), a['score'], a['score_adj'], a['tag'], a['source']])
        print('Wrote matching CSV to', out)
        if args.match_apply:
            # construct a CSV that apply_candidates_from_csv can read
            apply_csv = Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_matching_apply.csv')
            with apply_csv.open('w', encoding='utf-8', newline='') as f:
                w = csv.writer(f)
                w.writerow(['edge_a', 'edge_b', 'count', 'candidate_idx', 'vector', 'score', 'confidence', 'tag', 'source', 'note', 'derived_from', 'suggested_apply', 'apply', 'comment'])
                for a in match['assignments']:
                    w.writerow([a['edge_a'], a['edge_b'], 0, 0, json.dumps(a['vector']), a['score'], 'auto', a['tag'], a['source'], 'matching assignment', '', 'yes', 'yes', 'matching'])
            print('Generated matching apply CSV', apply_csv)
            if args.dry_run:
                print('Dry-run: not applying matching')
            else:
                res = apply_candidates_from_csv(apply_csv, combined_map_path=COMBINED_MAP, dry_run=args.dry_run, force=args.force)
                print('Apply result:', res)
    ...
    if args.apply:
        ...
        res = apply_candidates_from_csv(csv_path, combined_map_path=COMBINED_MAP, dry_run=args.dry_run, force=args.force)
        print('Apply result:', res)
