...
def find_simple_roots_in_a2(a2_indices: List[int], root_index_to_coords: Dict[int, Tuple[int, ...]]):
...
def main():
    ...
    p.add_argument('--top-edges-json', type=Path, default=Path('analysis/minimal_commutator_cycles/problem_cycle_edge_tally.json'))
    p.add_argument('--combined-map', type=Path, default=Path('artifacts/edge_to_e8_root_combined.json'))
    p.add_argument('--cycles-json', type=Path, default=Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_results_cycles_for_e8.json'))
    ...
    p.add_argument('--baseline-kind', choices=['auto', 'combined', 'postapply'], default='auto', help='Baseline per-A2 outputs to use (default: auto)')
    p.add_argument('--score-mode', choices=['fixed', 'net'], default='fixed', help='Score candidates by fixed matches or net (fixed-regressed)')
    p.add_argument('--recompute-top-edges', action='store_true', help='Ignore --top-edges-json and recompute from baseline problem cycles')
    p.add_argument('--top-n', type=int, default=20)
    p.add_argument('--candidate-nearest', type=int, default=100)
    p.add_argument('--apply-threshold', type=int, default=3)
    p.add_argument('--out-json', type=Path, default=Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_swap_candidates.json'))
    p.add_argument('--out-csv', type=Path, default=Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_swap_candidates.csv'))
    p.add_argument('--out-apply-csv', type=Path, default=Path('analysis/minimal_commutator_cycles/w33_uv_parser_det1_swap_candidates_vetting.csv'))
    ...
    root_index_map = {}
    try:
        er_data = json.loads(Path('artifacts/edge_root_bijection_canonical.json').read_text(encoding='utf-8'))
        ...
        pass
    # load a2 solutions
    a2_sols = json.loads(Path('artifacts/a2_4_decomposition.json').read_text(encoding='utf-8'))['a2_4_solution']
    ...
    if baseline_kind == 'auto':
        postapply_ok = all(Path(f'analysis/minimal_commutator_cycles/e8_det1_postapply_a2_{i}/e8_rootword_cocycle.json').exists() for i in range(4))
        baseline_kind = 'postapply' if postapply_ok else 'combined'

    per_a2_rows = []
    for i in range(4):
        pth = Path(f'analysis/minimal_commutator_cycles/e8_det1_{baseline_kind}_a2_{i}/e8_rootword_cocycle.json')
        if not pth.exists():
    ...
    if ncycles != len(cycles):
        raise RuntimeError(f'Baseline cycle count mismatch: per_a2_rows={ncycles} vs cycles_json={len(cycles)}')
    ...
    problem_cycle_set = {i for i in range(ncycles) if baseline_any_divisible[i] and not baseline_any_match[i]}
    ...
    for a2_idxs in a2_pairs:
        try:
            alpha, beta, _ai_idx, _bi_idx = find_simple_roots_in_a2(a2_idxs, root_index_map)
        except Exception:
    ...
    for a,b,count in top_edges:
        ...
        neighbors = sorted(canonical_roots, key=lambda r: dist2(r, current) if current else 0)[:args.candidate_nearest]
        for r in neighbors:
            ...
            affected_cycle_indices = sorted(set(edge_to_cycles.get(key, [])) | set(edge_to_cycles.get(rev_key, [])))
                ...
                if k_val is not None:
                    for simple in a2_simple_roots:
                        if simple is None:
                            continue
                        alpha, beta = simple
                        tvals = [inner_prod_doubled(rr, alpha) - inner_prod_doubled(rr, beta) for rr in rlist]
                        S = sum(tvals)
                        if S % 3 != 0:
                            continue
                        s_mod3 = (S // 3) % 3
        ...
        if args.score_mode == 'fixed':
            candidates.sort(key=lambda c: (-c['fixed_matches'], c.get('regressed_matches', 0), -c.get('net_matches', 0)))
        else:
            candidates.sort(key=lambda c: (-c.get('net_matches', 0), -c['fixed_matches'], c.get('regressed_matches', 0)))
        out_rows.append({'edge': f'{a},{b}', 'count': count, 'candidates': candidates[:10]})
        for c in candidates:
            score = c['fixed_matches'] if args.score_mode == 'fixed' else c.get('net_matches', 0)
            if score >= args.apply_threshold:
                ...
                vet_rows.append({'edge_a': a, 'edge_b': b, 'vector': json.dumps(c['vector']), 'score': score, 'comment': comment})

    # write outputs
    out_json = args.out_json
    out_csv = args.out_csv
    out_json.write_text(json.dumps({'top_edges_analyzed': len(top_edges), 'out_rows': out_rows}, indent=2), encoding='utf-8')
    with out_csv.open('w', encoding='utf-8', newline='') as f:
        w = csv.writer(f)
        w.writerow(['edge', 'count', 'top_candidate_vector', 'top_fixed_matches', 'top_regressed_matches', 'top_net_matches', 'score_mode'])
        for ent in out_rows:
    ...
    with vet_csv.open('w', encoding='utf-8', newline='') as f:
        ...
        for r in vet_rows:
            w.writerow([r['edge_a'], r['edge_b'], 0, 0, r['vector'], r['score'], 'auto', 'swap-canon', 'swap', r.get('comment',''), '', 'yes', '', r.get('comment','')])
...
if __name__ == '__main__':
    try:
        ...
        _P('tmp/swap_candidate_impact_error.txt').write_text(traceback.format_exc(), encoding='utf-8')
        traceback.print_exc()
        raise
