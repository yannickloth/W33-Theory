...
def find_simple_roots_in_a2(a2_indices, root_index_to_coords):
    roots = {i: root_index_to_coords[i] for i in a2_indices if i in root_index_to_coords}
...
def main():
    cycles = json.loads(Path('analysis/minimal_commutator_cycles/minimal_holonomy_cycles_ordered_rootwords.json').read_text(encoding='utf-8'))
    cycle = cycles[0]
    ...
    for ent in json.loads(Path('artifacts/edge_root_bijection_canonical.json').read_text(encoding='utf-8')):
        idx=int(ent['root_index']); coords=tuple(int(x) for x in ent['root_coords'])
        if idx not in root_index_map:
            root_index_map[idx]=coords

    a2_data=json.loads(Path('artifacts/a2_4_decomposition.json').read_text(encoding='utf-8'))
